...
| **MUSC-F8 — Notifications push muscu** | 3.42, 2.7, 2.4 | Push nouveau record + rappel de séance planifiée. | ⬜ L'infra notif existe (streak, DND) → à étendre. **Absorbe** l'idée « rappels contextuels » côté muscu. L'**échéance apprise** (p90) et le rabattement DND sont livrés par NUTR-F1 (`learned-hour.ts`, `clampOutOfDnd`) et **réutilisables tels quels** — ne pas les reconcevoir. ⚠️ Lire aussi les décisions **D8** (marge d'imminence de 15 min) et **D9** (clé du jour réactive) de sa spec : les deux mêmes pièges attendent tout nouveau rappel. |
