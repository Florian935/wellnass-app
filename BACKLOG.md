...
| ~~**Progression au niveau du programme (roadmap 3.7)**~~ | 3.7 | — | 🟡 **Entrée dans le pipeline le 02/08/2026** → [spec MUSC-F15](docs/specs/functional/us/muscf15-progression-programme.md) · [plan](docs/plans/muscf15-progression-programme.md) · [maquette](design/muscf15-progression-programme/muscf15-progression-programme.html). Cadrage tranché : **aucune cible évolutive stockée** — un second gate sur `computeProgressionSuggestion` (comme `previousStruggled`/MUSC-F7), évalué à la volée à partir du taux de complétion de la semaine `week_index − 1` du même programme (≥ 80 %, seuil déjà donné par la roadmap). Aucune migration.
