...
| ~~**RUN-F2c — Blocs fractionné/intervalles**~~ | 5.9 | — | 🟡 **Entrée en pipeline le 02/08/2026** → [spec](docs/specs/functional/us/runf2c-blocs-fractionne.md) · [plan](docs/plans/runf2c-blocs-fractionne.md) · [maquette](design/runf2c-blocs-fractionne/runf2c-blocs-fractionne.html). Modèle : une ligne = un bloc de répétitions (comme `exercise_plans.target_sets`), nouvelle table `session_intervals`. ⚠️ **2 sync rules à déployer manuellement sur le dashboard PowerSync** (table neuve, pas une simple colonne). |
