...
| ~~**RUN-F2a — Annonces audio périodiques**~~ | 5.19 | — | 🟡 **Entrée en pipeline le 02/08/2026** → [spec](docs/specs/functional/us/runf2a-annonces-audio.md) · [plan](docs/plans/runf2a-annonces-audio.md) · [maquette](design/runf2a-annonces-audio/runf2a-annonces-audio.html). Nécessite `expo-speech` (absent) → nouveau dev build EAS requis pour la recette (comme `expo-haptics`/MUSC-F9). |
| **RUN-F2b — Prolonger ou raccourcir** | 5.23 | Terminer avant la cible ou continuer en libre. | ⬜ `run/active.tsx` ne lit aujourd'hui aucune cible pendant la course (`useRunTarget` n'existe que côté résumé, post-course, US RUN-F3). Non cadrée. |
| **RUN-F2c — Blocs fractionné/intervalles** | 5.9 | Blocs rapides/récupération structurés (ex. 6×400 m à 95 % VMA). | ⬜ **Le plus gros morceau** : aucune notion de bloc n'existe (ni colonne, ni table, ni éditeur admin, ni éditeur mobile) — `fractionne` n'est qu'une étiquette avec une plage d'allure globale. Demande une nouvelle table liée à `sessions` + 2 éditeurs (admin + mobile) + migration. Non cadrée. |
| **RUN-F2d — Guidage fractionné vocal** | 5.18 | Annonce vocale + vibration à chaque changement de bloc. | ⬜ **Dépend entièrement de RUN-F2a (vocal) et RUN-F2c (blocs)** — à cadrer seulement après elles. Aucun précédent de vibration périodique en tâche de fond dans le projet (le seul cas existant, `workout.tsx`, est en foreground actif). |
