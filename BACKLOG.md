...
| ~~**Widget écran d'accueil Android**~~ | [IDEAS.md](IDEAS.md) 13/07 | — | 🟡 **Entré dans le pipeline le 03/08/2026** → [spec](docs/specs/functional/us/launcher01-widget-ecran-accueil.md) · [plan](docs/plans/launcher01-widget-ecran-accueil.md) · [maquette](design/launcher01-widget-ecran-accueil/launcher01-widget-ecran-accueil.html), **en attente de validation Florian/Damien** (roadmap 7.19, LAUNCHER-01). Coût réel révisé à la baisse : `react-native-android-widget` (JSX → RemoteViews, config plugin Expo) évite d'écrire du Kotlin à la main — reste à confirmer par un spike de compatibilité SDK 57. |
