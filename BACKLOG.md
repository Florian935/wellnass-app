...
|---|---|---|---|
| ~~**MUSC-F7**~~ | 3.8 | — | 🟡 **Entré dans le pipeline le 01/08/2026** → [spec](docs/specs/functional/us/muscf7-progression-assistee.md) · [plan](docs/plans/muscf7-progression-assistee.md). **Pas de maquette** : la brique de calcul, l'UI de restitution et les 2 clés i18n existent **déjà** (Refonte-C3) — il ne manque que le signal `previousStruggled` (séance avant-dernière), jamais fourni à l'appel. **Scindé du roadmap 3.7** (voir ligne 3.7 ci-dessous) : la « progression au niveau programme » est un chantier de conception à part (aucune brique de données), pas un sous-lot de ce câblage. |
