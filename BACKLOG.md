...
| ~~**RUN-F1b — Dénivelé cumulé**~~ | 5.32 | — | 🟡 **Blocage levé, entrée en pipeline le 02/08/2026** → [spec](docs/specs/functional/us/runf1b-denivele-cumule.md) · [plan](docs/plans/runf1b-denivele-cumule.md) · [maquette](design/runf1b-denivele-cumule/runf1b-denivele-cumule.html). Le diagnostic ⛔ supposait qu'il fallait étendre le codec de trace — évité : deux scalaires cumulés en direct par le tracker (comme `distance_m`/`duration_seconds`), `gps_track` inchangé. Les courses déjà enregistrées restent sans dénivelé (donnée source absente), mais ce n'est plus une limite du format.
