...
| **Progression au niveau du programme (roadmap 3.7)** | 3.7 | Cible de charge recalculée d'une semaine à l'autre si ≥ 80 % complété. | 🔴 **Scindé de MUSC-F7 le 01/08/2026** — cartographie du 01/08 : `exercise_plans.target_weight_kg` est une valeur unique et figée par plan, aucun taux de complétion par semaine n'est calculé nulle part, `planned_sessions.week_index` n'est posé qu'une fois à la génération du planning. **Rien de tout ça n'existe** : il faut un vrai cadrage produit (où stocker une cible qui évolue ? quelle fenêtre « semaine » ? quand déclencher le recalcul ? comment l'exposer ?) avant tout code — pas juste un signal manquant comme 3.8. L'estimation roadmap actuelle (« Moyen, 3h ») est sous-évaluée pour ce périmètre réel. |
...
| ~~**MUSC-F1b**~~ | 6.2 | — | 🟡 **Recadré en Voie B le 01/08/2026** (Florian) → [spec](docs/specs/functional/us/muscf1b-schema-muscles.md) · [plan](docs/plans/muscf1b-schema-muscles.md) · [maquette](design/muscf1b-schema-muscles/muscf1b-schema-muscles.html). Anatomie fine sur **10 muscles** (référentiel repris d'`administration.md` §3.3, jamais construit), **additif** aux 6 groupes larges — aucun ricochet sur les 18 fichiers qui les consomment déjà (alerte déséquilibre, filtre, remplacement, graphique de volume). Repli automatique sur les groupes larges tant qu'un exercice n'est pas tagué fin par un coach (hors dev, ~1-2h pour les 16 exercices). En attente de validation de la maquette (relecture anatomique des 11 tracés, critère de recette 12) avant le code. |
