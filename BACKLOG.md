...
| ~~**REFACTO-01**~~ | 9.16 | — | 🟡 **Entré dans le pipeline le 31/07/2026** → [spec](docs/specs/functional/us/refacto01-acces-pilier.md) · [plan](docs/plans/refacto01-acces-pilier.md). **Pas de maquette** (refactor invisible, aucun écran touché). Cartographie complète faite : ~10 sites recopient `settings?.activePillars ?? [...PILLARS]`, dont un **désynchronisé de `PILLARS`** dans `weekly-review-repository.ts` (repli codé en dur `['strength','running','nutrition']`) — bug latent corrigé au passage. Périmètre volontairement étroit : ni le `WidgetGuard` de `widgets.ts`, ni les 2 sites de conjonction (`&&`), ni `apps/admin/.../users.ts` (repli inversé) ne sont touchés — zéro bénéfice mesurable à les unifier. **Zéro changement de comportement à 3 piliers** → pas de recette device. |
