...
| ~~**MUSC-F6**~~ | 3.36 | — | 🟡 **Entré dans le pipeline le 31/07/2026** → [spec](docs/specs/functional/us/muscf6-fenetre-reprise-seance.md) · [plan](docs/plans/muscf6-fenetre-reprise-seance.md). **Pas de maquette** pour l'option recommandée (doc uniquement). Le « conflit 3h/4h » n'a **jamais existé dans le comportement observable** : `WORKOUT_AUTO_CLOSE_SECONDS` (3h, déjà testée) est la seule limite réelle — la promesse « 4h + popup Pause » de `musculation.md` §4.4 n'a jamais été implémentée (aucun statut `paused`, aucune constante, aucune chaîne i18n). **Décision D1 à valider** : Option A (officialiser 3h, doc uniquement, recommandée) vs Option B (fenêtre distincte de 4h + avertissement — vrai chantier, bénéfice non démontré). |
