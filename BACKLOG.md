...
| ~~**RUN-F2b — Prolonger ou raccourcir**~~ | 5.23 | — | 🟡 **Entrée en pipeline le 02/08/2026** → [spec](docs/specs/functional/us/runf2b-cible-en-direct.md) · [plan](docs/plans/runf2b-cible-en-direct.md) · [maquette](design/runf2b-cible-en-direct/runf2b-cible-en-direct.html). Reformulation du besoin : « terminer avant »/« continuer en libre » sont déjà natifs (Stop existant, rien ne bloque la poursuite) — le vrai manque est la **visibilité** de la cible pendant la course. Réutilise `compareToTarget`/`useRunTarget`/`running.target.*` de RUN-F3 tels quels, aucune fonction ni clé neuve. |
