/**
 * Le calendrier du mois de l'Historique Course — US CARDIO-UX03, R8 et R9.
 *
 * Retrouver « la sortie de dimanche » d'un coup d'œil : une grille lundi → dimanche (jours locaux),
 * les jours de sortie pleins, un repère pour celles qui détiennent encore un record, les séances de
 * course prévues à venir en pointillé, aujourd'hui cerclé. Un jour à une sortie ouvre son détail ; à
 * plusieurs, il restreint la liste en dessous. Jours vides et prévus ne sont pas des boutons.
 *
 * La grille elle-même vient de `buildMonthGrid` (`history-calendar.ts`, écrite pour la muscu et déjà
 * agnostique du pilier, réutilisée sans modification). Le composant est, lui, **dupliqué** de
 * `HistoryCalendar` : son résumé dit des tonnes, celui-ci des kilomètres (spec §11 : la factorisation
 * viendra quand les trois hubs auront atterri).
 */
...
export type CalendarMonthRun = { dayKey: string; recordCount: number };

type Props = {
  month: YearMonth;
  range: { min: YearMonth; max: YearMonth };
  runs: readonly CalendarMonthRun[];
  summary: { count: number; distanceM: number; durationSeconds: number; records: number };
  plannedDayKeys: readonly string[];
  todayKey: string;
  selectedDayKey: string | null;
  onMonth: (month: YearMonth) => void;
  onDay: (dayKey: string) => void;
  /** Nomme les sorties d'un jour, pour TalkBack (« Fractionné (VMA), 7,40 km »). */
  describeDay: (dayKey: string) => string;
};

const WEEKDAY_KEYS = ['mon', 'tue', 'wed', 'thu', 'fri', 'sat', 'sun'] as const;

export function RunHistoryCalendar({
  month,
  range,
  runs,
  summary,
  ...
  const grid = buildMonthGrid({ ...month, workouts: runs, plannedDayKeys, todayKey });
  ...
  const arrow = (dir: 'prev' | 'next', enabled: boolean) => (
    <Pressable
      testID={`run-calendar-${dir}`}
      onPress={() => enabled && shift(dir === 'prev' ? -1 : 1)}
      disabled={!enabled}
      accessibilityRole="button"
      accessibilityLabel={t(dir === 'prev' ? 'runningHub.history.previous' : 'runningHub.history.next')}
  ...
  const renderCell = (cell: CalendarCell, index: number) => {
    ...
    const ran = cell.state === 'done' || cell.state === 'record';
    ...
    if (ran) {
      return (
        <View key={dayKey} style={styles.cell}>
          <Pressable
            testID={`run-calendar-day-${dayKey}`}
            onPress={() => onDay(dayKey)}
            accessibilityRole="button"
            accessibilityState={{ selected }}
            accessibilityLabel={t('runningHub.history.dayA11y', {
              date: new Date(`${dayKey}T12:00:00`).toLocaleDateString(i18n.language, {
                weekday: 'long',
                day: 'numeric',
                month: 'long',
              }),
              runs: describeDay(dayKey),
            })}
            style={[
              styles.dot,
              { backgroundColor: colors.accent },
              cell.isToday && { borderWidth: 2, borderColor: colors.text },
  ...
  const line =
    summary.count > 0
      ? [
          t('runningHub.history.runs', { count: summary.count }),
          units.formatDistance(summary.distanceM / 1000),
          formatHoursMinutes(summary.durationSeconds),
        ].join(' · ')
      : t('runningHub.history.empty');

  return (
    <View testID="run-history-calendar" style={[styles.card, { backgroundColor: colors.surface, borderColor: colors.border }]}>
      <View style={styles.head}>
        ...
        <View style={styles.headText}>
          ...
          <Text testID="run-calendar-summary" style={[styles.summary, { color: colors.textMuted }]}>
            {line}
          </Text>
          {summary.records > 0 ? (
            <Text style={[styles.records, { color: colors.warnText }]}>
              {t('runningHub.history.records', { count: summary.records })}
            </Text>
          ) : null}
        </View>
        {arrow('next', canNext)}
      ...
      <View style={styles.legend} importantForAccessibility="no-hide-descendants">
        <View style={styles.legendItem}>
          ...
          <Text style={[styles.legendText, { color: colors.textMuted }]}>{t('runningHub.history.legend.run')}</Text>
        </View>
        <View style={styles.legendItem}>
          <View style={[styles.legendDot, { backgroundColor: colors.amber }]} />
          <Text style={[styles.legendText, { color: colors.textMuted }]}>{t('runningHub.history.legend.record')}</Text>
        </View>
        <View style={styles.legendItem}>
          <View style={[styles.legendDot, { borderWidth: 2, borderStyle: 'dashed', borderColor: colors.accent }]} />
          <Text style={[styles.legendText, { color: colors.textMuted }]}>{t('runningHub.history.legend.planned')}</Text>
        </View>
      </View>
    </View>
  );
