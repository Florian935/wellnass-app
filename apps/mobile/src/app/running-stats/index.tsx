/**
 * « Toutes tes stats » — les analyses détaillées de la course (US CARDIO-UX03, D7 ; Q8).
 *
 * C'était l'écran « Historique & progression » (R4a — phase D), `/running-history`, qui empilait la
 * liste des courses entre les statistiques et les records : la liste était en troisième section,
 * plate, sans type de séance, et ouvrait l'écran d'arrivée. Depuis CARDIO-UX03, la liste vit dans
 * l'onglet Historique du hub (calendrier, lignes enrichies, par type), et `/running-history` y
 * redirige. Ici restent les analyses, **telles quelles**, ouvertes depuis l'onglet Progrès (comme
 * `/progress` pour la muscu) :
 ...
 *  3. Records d'allure (un record ouvre le détail de sa sortie, l'analyse — plus l'écran d'arrivée),
 *     objectifs estimés, charge d'entraînement, polarisation.
 *
 * Conventions :
...
export default function RunningHistoryScreen() {
  ...
  // s'affiche comme un faux « aucune course » (0/0/0 + courbe vide).
  ...
  return (
    <Screen edges={['top']}>
      <ScreenHeader title={t('runningStats.title')} />
      ...
      {CANONICAL_RUN_RECORD_ROWS.map(({ key, meters }) => {
        ...
        return (
          <Pressable
            ...
            onPress={() => router.push({ pathname: '/run/analysis', params: { id: record.runId } })}
            style={({ pressed }) => [
              styles.runRow,
