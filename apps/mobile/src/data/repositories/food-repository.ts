...
// ---------------------------------------------------------------------------
// useFoodsByNames — US DASH-01 (§7.2, photo du repas)
// ---------------------------------------------------------------------------

/** Au-delà, ce n'est plus une assiette : c'est un modèle qui déraille (cap de `mealPhotoResultSchema`). */
const MAX_LOOKUP_NAMES = 12;

/**
 * Les aliments du catalogue **portant les noms rendus par le modèle**, pour la photo de repas.
 *
 * C'est ce hook qui fait que **rien de ce qui est affiché ne vient d'un modèle** : le modèle donne
 * un nom et des grammes, le catalogue donne les valeurs, et la multiplication se fait ici. Un nom
 * introuvable ne produit aucune ligne — mieux vaut un aliment manquant qu'un aliment inventé (R5).
 *
 * Rapprochement volontairement **large** (`LIKE %nom%`, insensible à la casse) : le modèle écrit
 * « poulet grillé » là où le catalogue dit « Poulet (blanc, cuit) ». Un `=` strict ne rapprocherait
 * quasiment rien ; c'est `matchPhotoItems` qui tranche ensuite, et l'utilisateur qui valide.
 */
export function useFoodsByNames(names: readonly string[]): {
  foods: FoodListItem[];
  isLoading: boolean;
} {
  const { i18n } = useTranslation();
  ...
  // Requête montée inconditionnellement (règle des hooks) : sans nom, la clause `1 = 0` ne ramène
  // rien plutôt que de ramener tout le catalogue.
  const clause =
    cleaned.length > 0
      ? cleaned.map(() => 'LOWER(COALESCE(tl.name, tfr.name)) LIKE ?').join(' OR ')
      : '1 = 0';

  const { data, isLoading } = useQuery<FoodListDbRow>(
    `${SELECT_FOODS} AND (${clause}) ${ORDER_BY_NAME} LIMIT 60`,
    [lang, ...cleaned.map((name) => `%${name}%`)],
  );
