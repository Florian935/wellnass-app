...
## 57. ACCUEIL-01 → 06 — Refonte de l'écran d'accueil (les cinq zones)

> Specs : [ACCUEIL-01](docs/specs/functional/us/accueil01-carte-maintenant.md) ·
> [02](docs/specs/functional/us/accueil02-entete-utile.md) ·
> [03](docs/specs/functional/us/accueil03-actions-rapides.md) ·
> [04](docs/specs/functional/us/accueil04-densification-grille.md) ·
> [05](docs/specs/functional/us/accueil05-pied-et-rafraichissement.md) ·
> [06](docs/specs/functional/us/accueil06-adr-et-filet-de-tests.md)
> [Plan](docs/plans/accueil-refonte.md) · Roadmap 7.23 → 7.28 · Branche `feature/accueil-refonte`
> Livré le **09/09/2026** · Analyse : `design/accueil-refonte/Refonte-accueil-analyse.pdf`
> Maquettes : canvas « Accueil FitTrio » (6 planches)
>
> **Six US, un seul lot** — à ta demande (« TOUT faire d'un seul gros lot, je ferai le recettage à
> la toute fin »). Les six specs restent séparées : tu peux clôturer les unes et recaler les autres.

### 🟢 Prérequis — il n'y en a presque pas

- [ ] 0a. **Aucune migration, aucune sync rule, aucune dépendance native.** Rien à pousser, rien à
      déployer dans le dashboard PowerSync. C'est le lot le plus léger de ce point de vue.
- [ ] 0b. 🔴 **L'APK doit être construit depuis ce lot ou plus récent.** Pas besoin d'un nouveau dev
      build EAS (aucune dépendance native neuve), mais le JS est figé dans l'APK release : un APK
      antérieur ne contient **rien** de la refonte.
      ...
      ⚠️ **La purge du bundle n'est pas optionnelle** : Gradle ne déclare pas `packages/shared`
      comme entrée, se croit à jour et réemballe l'ancien JS — `BUILD SUCCESSFUL` avec la refonte
      absente. Contrôle avant d'envoyer l'APK, depuis la racine :

      ```powershell
      Select-String -SimpleMatch -Pattern "resolveNowAction" `
        apps/mobile/android/app/build/generated/assets/react/release/index.android.bundle
      ```

### 🔴 LE critère qui compte plus que tous les autres

- [ ] 1. **Ouvrir l'app avec un accueil déjà personnalisé** (widgets déplacés, tailles changées avant
      ce lot) → **aucun widget superposé, aucun trou, ordre préservé**.

      C'est le seul risque sérieux du lot : la résolution verticale de la grille a **doublé**
      (`sizeSpan` compte désormais en demi-cases). La migration est automatique et couverte par
      4 tests, mais elle porte sur des données réelles enregistrées dans
      `user_settings.dashboard_layout` — et le symptôme d'un échec serait immédiatement visible.
      Si quelque chose cloche ici, **arrête la recette et dis-le** : tout le reste est cosmétique
      à côté.

### Zone 0 — l'en-tête (ACCUEIL-02)

- [ ] 2. La **date du jour** s'affiche en clair en haut de l'écran.
- [ ] 3. Le **nom de l'application** n'apparaît **plus** en titre.
- [ ] 4. L'accroche sous la date correspond à ce que dit la carte en dessous — jamais de
      contradiction entre les deux.
- [ ] 5. L'avatar en haut à droite ouvre les réglages ; la pastille de synchro est toujours lisible.
- [ ] 6. « Personnaliser » **n'est plus** en haut à droite — il est descendu en bas de l'écran.

### Zone 1 — la carte « maintenant » (ACCUEIL-01)

- [ ] 7. Un jour avec **séance muscu planifiée à une heure** : la carte affiche le nom **et l'heure**.
- [ ] 8. « Démarrer la séance » ouvre la séance. **Double appui rapide → une seule séance créée.**
- [ ] 9. 🔴 Un jour avec **sortie de course planifiée et aucune séance muscu** : la carte affiche la
      **course**. C'était le défaut principal — avant ce lot, elle disait « Rien de prévu ».
- [ ] 10. « Démarrer la course » ouvre bien le suivi GPS sur la séance planifiée.
- [ ] 11. **Deux séances le même jour** (une muscu, une course) : c'est **la plus proche en heure**
      qui s'affiche.
- [ ] 12. **Pendant** une séance en cours : la carte propose « Reprendre », **jamais** « Démarrer ».
- [ ] 13. Le soir, **repas non saisi** après ton heure habituelle : la carte le réclame et annonce
      « d'habitude vers X h » (c'est l'heure **apprise**, jusqu'ici invisible dans l'app).
- [ ] 14. Après une journée complète : la carte **rend compte** (séances, série) et ne propose
      **aucun bouton**.
- [ ] 15. Journée **totalement vide** à 22 h : la carte ne prétend **pas** que la journée est faite.

### Zone 2 — les actions rapides (ACCUEIL-03)

- [ ] 16. Les quatre pastilles sont visibles et chacune mène au bon écran.
- [ ] 17. 🔴 Le libellé de la pastille repas **change avec l'heure** : « Petit-déj » le matin,
      « Déjeuner » à midi, « Dîner » le soir.
- [ ] 18. Un appui sur **« + Repas » à 20 h** ouvre le **dîner** — plus jamais le petit-déjeuner.
- [ ] 19. Un appui sur la **carte nutrition** ouvre le **même** repas que la pastille.
- [ ] 20. Quand la carte du haut réclame un repas, la pastille correspondante est **en accent**.
- [ ] 21. Désactiver le pilier **Course** dans les réglages → la pastille « Course » disparaît.
- [ ] 22. Cibles tactiles confortables **à bout de bras** (test en salle, main humide).

### Zone 3 — la grille densifiée (ACCUEIL-04)

- [ ] 23. **Pas et poids côte à côte** sur une seule ligne (deux petits carrés).
- [ ] 24. Le widget **poids** est de retour : chiffre, tendance 7 jours, courbe.
- [ ] 25. Sans aucune pesée enregistrée, la carte poids affiche son **état vide** — pas un zéro.
- [ ] 26. La carte **« vie réelle » hors période** est une **bande fine**, pas un grand rectangle
      presque vide.
- [ ] 27. 🔴 **Déclarer une période « vie réelle »** → la carte **grandit** et ses deux boutons sont
      accessibles. La période finie → elle **redevient une bande**, et ta disposition n'a pas bougé.
- [ ] 28. La carte **nutrition** affiche les **trois macros** avec leur progression.
- [ ] 29. Un appui sur **Régularité** ouvre le **bilan de la semaine** (la carte n'était pas
      tappable avant).
- [ ] 30. 🔴 À l'ouverture de l'app, **l'écran ne saute pas** : les cellules sont en place dès le
      départ, seul leur contenu se remplit.
- [ ] 31. **Aucun « 0 » ne clignote** pendant le chargement (ni série, ni calories, ni pas).
- [ ] 32. 🔴 **Mode édition** : déplacer un widget le pose **exactement** là où le doigt le lâche —
      pas une demi-cellule plus bas. À vérifier sur les **quatre** formes (bande, petit carré,
      rectangle, grand carré).
- [ ] 33. Le sélecteur de forme propose bien **quatre** tailles, et le rendu en **bande** tient sur
      une ligne pour chaque widget.
- [ ] 34. Masquer puis réafficher un widget : **aucun trou** dans la grille.

### Zone 4 — le pied et le rafraîchissement (ACCUEIL-05)

- [ ] 35. Le pied affiche les **prochaines** séances (jamais celle d'aujourd'hui), avec leur date.
- [ ] 36. « Voir mon planning » ouvre le planning ; « Personnaliser » entre en mode édition.
- [ ] 37. **Tirer vers le bas** : l'indicateur apparaît, tourne brièvement, disparaît.
- [ ] 38. En **mode avion**, le même geste ne produit **aucune alerte** et s'arrête de lui-même.
- [ ] 39. Couper puis rétablir le réseau, tirer → la synchro reprend (pastille « À jour »).
- [ ] 40. En **mode édition**, le tirage ne déclenche rien (le geste sert au déplacement).

### Transverse

- [ ] 41. **Basculer l'app en anglais** : tout l'accueil est traduit — date, accroche, carte,
      pastilles, pied. Aucune chaîne en français résiduelle.
- [ ] 42. **Grande taille de police système** (réglages Android) : rien n'est tronqué, l'accroche
      passe sur deux lignes au maximum.
- [ ] 43. **Thème sombre** : la carte « maintenant » reste lisible, le libellé de son bouton est
      foncé sur l'accent (règle CONF-07 D1).
- [ ] 44. Un utilisateur **mono-pilier** (muscu seule) : l'accueil reste cohérent, sans pastille
      repas ni course, sans carte nutrition.

### 🟠 Deux arbitrages à confirmer

- [ ] 45. **Le registre reste à 8 widgets**, `MAX_HOME_WIDGETS` inchangé. Le compte rendu que je
      t'ai remis recommandait 8 → 9 pour le retour du poids ; la promotion de `today-session` en
      zone épinglée a rendu ce +1 inutile. **C'est une meilleure issue** (le cliquet posé par
      INSIGHTS-02 n'est pas consommé) mais c'est un écart à ce que tu as validé : à confirmer.
...
- **La ligne « objectif personnel » du pied de page**, présente sur la maquette validée. Un
  `PersonalGoal` ne porte **ni titre ni libellé** : la phrase lisible est composée par la carte
  d'objectif de `/goals`. En écrire une seconde version pour une ligne de 13 px serait la
  duplication qu'ADR-007 §3 proscrit. À reprendre quand ce libellé sera une brique réutilisable —
  la place est prévue, la ligne s'ajoutera sans rien déplacer.
- **Le halo d'accent.** L'analyse le comptait parmi les 20 défauts (« appliqué à toutes les
  cartes ») : **c'était faux**, et je l'ai corrigé avant de coder. `hasHaloFor` n'en pose que sur
  ~1 widget sur 3 depuis la roadmap 7.22, avec exactement le raisonnement que je proposais. Rien
  n'a été touché — mais si tu trouves le résultat trop discret ou trop présent, c'est là qu'il
  faut regarder.
- **Les 36 tests en échec de `ExerciseEditScreen.test.tsx`** (back-office). Vérifié : ils
  échouent **déjà sur `dev` sans ce lot**. Régression préexistante, à traiter séparément.
