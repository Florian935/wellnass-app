// build.mjs (v3) — pipeline complet : anatomie SDF (îlots + sillons) → surface nets → 14 morphs → contrôles
// → GLB sparse (single + split) → planches PNG de contrôle (repos, sillons, extrêmes EN DÉCOUPÉ, comparaison v2).
// Usage : node build.mjs [h=0.010] [--no-render] [--out=out]
import fs from 'node:fs';
import path from 'node:path';
import { fileURLToPath } from 'node:url';
import { buildBody, GRID } from './lib/body.mjs';
import { extract } from './lib/surfacenets.mjs';
import { MORPH_NAMES, MORPHS_BY_PART, prepareVertexData, computeDeltas, tagTriangles, enforceSeams, repairFlips, Y_SEAM_LEGS, SEAM_UT_BACK, SEAM_UT_FRONT, seamUT } from './lib/morphs.mjs';
...
const h = Number(args.find((a) => !a.startsWith('--')) ?? 0.011);
...
console.log(`Anatomie : ${body.prims.length} primitives (${body.prims.length - nSub} volumes, ${nSub} capsules de sillon) ; grille h=${h} m (${mesh.grid.nx}×${mesh.grid.ny}×${mesh.grid.nz}) ; ${V} sommets, ${T} triangles — ${Date.now() - t0} ms`);
console.log(`  soudure : ${mesh.grid.merged} sommets fusionnés, ${mesh.grid.dropped} triangles dégénérés retirés ; nettoyage : ${mesh.grid.removedComponents} composante(s) < 50 tris retirée(s) (${mesh.grid.removedTriangles} tris), ${mesh.grid.orphanVertices} sommet(s) orphelin(s) retiré(s)`);
...
console.log(`  réparation ciblée : ${repaired} micro-triangle(s) corrigé(s) (deltas moyennés sur les 3 sommets)`);
console.log(`Morphs calculés en ${Date.now() - t0} ms ; parties : ${['upper', 'trunk', 'legs'].map((p) => `${p} ${tags.filter((t) => t === p).length} tris`).join(', ')} ; coutures : haut/tronc inclinée ${SEAM_UT_BACK} (dos) → ${SEAM_UT_FRONT} (face), tronc/jambes y=${Y_SEAM_LEGS}`);
...
else console.log('  jonctions : aucun delta à écraser (les fenêtres s\'arrêtent avant les coutures)');
...
const COMBOS = [
  ['tout +1', () => 1],
  ['props −1, goals +1', (n) => (n.startsWith('prop_') ? -1 : 1)],
  ['props −1', (n) => (n.startsWith('prop_') ? -1 : 0)],
  ['props +1', (n) => (n.startsWith('prop_') ? 1 : 0)],
  ['goals +1', (n) => (n.startsWith('goal_') ? 1 : 0)],
];
...
// mesures aux extrêmes (silhouette) : largeur d'épaules / taille / hanches / cuisse pour juger la crédibilité
...
const sp1 = writeSparseGlb([{ name: 'body', positions, normals, indices, morphs: singleMorphs }], 'mesh-v3 build.mjs (sparse)');
const fSingle = path.join(OUT, 'body-v3-single.glb');
...
const sp3 = writeSparseGlb(splitMeshes, 'mesh-v3 build.mjs (sparse, split)');
const fSplit = path.join(OUT, 'body-v3-split.glb');
...
fs.writeFileSync(path.join(OUT, 'morph-map.json'), JSON.stringify({
  version: 'v3', vertices: V, triangles: T, grid: h, morphs: MORPH_NAMES, morphsByPart: MORPHS_BY_PART,
  seams: { upperTrunk: { back: SEAM_UT_BACK, front: SEAM_UT_FRONT, note: 'surface seamUT(x,z) le long du rebord costal : 1.12 au centre du dos, 1.19 sur les flancs, 1.20 devant' }, trunkLegs: Y_SEAM_LEGS, note: 'membre supérieur (bras + mains) toujours dans upper' },
  weightMapping: { 'prop_*': 'poids = valeur / 2 ∈ [-1,+1]', 'goal_*': 'poids = valeur / 4 ∈ [0,1]' },
  files: { single: 'body-v3-single.glb', split: 'body-v3-split.glb' },
  sizes: { single: fs.statSync(fSingle).size, split: fs.statSync(fSplit).size },
...
if (doRender) {
  ...
  // le maillage DÉCOUPÉ morphé : chaque partie applique ses propres cibles (comme l'app en mode split),
  // puis les trois sont assemblées pour le rendu — c'est là que les 14 s'appliquent vraiment
  ...
  savePng(sheet(MUSCLE_VIEWS.map(([, o]) => render(mesh, { width: 380, height: 420, ...o })), MUSCLE_VIEWS.map((v) => v[0])), path.join(OUT, 'ctrl-muscles.png'));
  ...
  // extrêmes EN DÉCOUPÉ (les 14 cibles s'appliquent vraiment) : repos, tout +1 (face, dos, 3/4), props −1, mixte
  ...
  savePng(sheet([
    render(morphedSplit({}), { ...small, yaw: 0 }), render(morphedSplit(all1), { ...small, yaw: 0 }), render(morphedSplit(all1), { ...small, yaw: 180 }), render(morphedSplit(all1), { ...small, yaw: 40 }),
    ...
    render(morphedSplit(propsPosOnly), { ...small, yaw: 0 }), render(morphedSplit(goalsOnly), { ...small, yaw: 0 }), render(morphedSplit(all1, false), { ...small, yaw: 0 }),
  ], ['REPOS SPLIT', 'TOUT +1 FACE', 'TOUT +1 DOS', 'TOUT +1 3/4', 'PROPS -1', 'PROPS -1 GOALS +1', 'MIXTE DOS', 'PROPS +1', 'GOALS +1', 'TOUT +1 NORM. BASE']), path.join(OUT, 'ctrl-extremes.png'));
  // gros plans des raccords aux extrêmes (couture hanches/cuisses, rebord costal), en découpé
  const seamViews = [
    ['HANCHES TOUT +1', { yaw: 20, target: [0.1, 0.8, 0], distance: 1.2, fov: 26 }],
    ['HANCHES DOS TOUT +1', { yaw: 200, target: [0.1, 0.8, 0], distance: 1.2, fov: 26 }],
    ['TAILLE TOUT +1', { yaw: 30, target: [0, 1.15, 0], distance: 1.3, fov: 26 }],
    ['TAILLE DOS TOUT +1', { yaw: 200, target: [0, 1.15, 0], distance: 1.3, fov: 26 }],
    ['HANCHES PROPS -1', { yaw: 20, target: [0.1, 0.8, 0], distance: 1.2, fov: 26 }],
    ['TAILLE PROPS -1', { yaw: 30, target: [0, 1.15, 0], distance: 1.3, fov: 26 }],
  ];
  ...
  // proportions : −1 | +1, de face (et de dos pour les mollets)
  ...
  savePng(sheet([render(mesh, pit), render(morphed({ prop_shoulders: -1 }), pit), render(morphed({ prop_shoulders: 1 }), pit), render(morphed({ goal_shoulders: 1, goal_chest: 1, goal_arms: 1 }), pit), render(morphed({ prop_shoulders: 1, prop_chest: 1, prop_arms: 1, goal_shoulders: 1, goal_chest: 1, goal_arms: 1, goal_back: 1 }), pit)],
    ['EPAULE REPOS', 'SHOULDERS -1', 'SHOULDERS +1', 'GOALS HAUT +1', 'HAUT TOUT +1']), path.join(OUT, 'ctrl-epaule.png'));
  // comparaison v2 (référence, h=0.014) / v3 au repos
  try {
    const v2body = (await import('./v2-ref/lib/body.mjs')).buildBody();
    const v2grid = (await import('./v2-ref/lib/body.mjs')).GRID;
    const v2mesh = (await import('./v2-ref/lib/surfacenets.mjs')).extract(v2body, { h: 0.014, ...v2grid });
    savePng(sheet([render(v2mesh, { ...big, yaw: 0 }), render(mesh, { ...big, yaw: 0 }), render(v2mesh, { ...big, yaw: 180 }), render(mesh, { ...big, yaw: 180 }), render(v2mesh, { ...big, yaw: 40 }), render(mesh, { ...big, yaw: 40 })],
      ['V2 FACE', 'V3 FACE', 'V2 DOS', 'V3 DOS', 'V2 3/4', 'V3 3/4']), path.join(OUT, 'ctrl-comparaison.png'));
  } catch (e) { console.log('  (comparaison v2 impossible : ' + e.message + ')'); }
