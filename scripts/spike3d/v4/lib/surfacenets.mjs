...
export function extract(body, { h, min, max, weld }) {
  ...
  const welded = weldVertices(pos, tris, weldEps);
  // nettoyage topologique : les sillons peuvent détacher des miettes (îlots de quelques triangles) et
  // certaines cellules produisent un sommet sans aucun quad (orphelin) — on ne garde que les
  // composantes ≥ 50 triangles, puis on compacte les sommets référencés
  const { positions, indices, removedComponents, removedTriangles, orphanVertices } = dropSmallComponents(welded.positions, welded.indices, 50);
  const { merged, dropped } = welded;
  ...
  return { positions, normals, indices, grid: { nx, ny, nz, h, samples: d.length, tSample, rawVertices: V, merged, dropped, removedComponents, removedTriangles, orphanVertices } };
}
...
/** Supprime les composantes connexes de moins de minTris triangles et les sommets non référencés. */
export function dropSmallComponents(positions, indices, minTris = 50) {
  const V = positions.length / 3;
  const parent = new Int32Array(V); for (let i = 0; i < V; i++) parent[i] = i;
  const find = (a) => { while (parent[a] !== a) { parent[a] = parent[parent[a]]; a = parent[a]; } return a; };
  for (let t = 0; t < indices.length; t += 3) { const a = find(indices[t]), b = find(indices[t + 1]); parent[a] = b; parent[find(b)] = find(indices[t + 2]); }
  const triCount = new Map();
  for (let t = 0; t < indices.length; t += 3) { const r = find(indices[t]); triCount.set(r, (triCount.get(r) ?? 0) + 1); }
  const keep = new Set([...triCount.entries()].filter(([, n]) => n >= minTris).map(([r]) => r));
  const removedComponents = triCount.size - keep.size;
  const remap = new Int32Array(V).fill(-1);
  const outPos = [], outIdx = [];
  let removedTriangles = 0;
  for (let t = 0; t < indices.length; t += 3) {
    if (!keep.has(find(indices[t]))) { removedTriangles++; continue; }
    for (let q = 0; q < 3; q++) {
      const v = indices[t + q];
      if (remap[v] < 0) { remap[v] = outPos.length / 3; outPos.push(positions[3 * v], positions[3 * v + 1], positions[3 * v + 2]); }
      outIdx.push(remap[v]);
    }
  }
  let referenced = 0; for (let v = 0; v < V; v++) if (remap[v] >= 0) referenced++;
  return { positions: new Float32Array(outPos), indices: Uint32Array.from(outIdx), removedComponents, removedTriangles, orphanVertices: V - referenced };
}
