...
// La version du modèle change : on vérifie la forme de la ligne, pas la version.
const claudeAttributionPattern = /Co-Authored-By: Claude [^<\n]+ <noreply@anthropic\.com>/;
const errors = [];
...
const skillAttribution = claudeSkill.match(claudeAttributionPattern)?.[0];
const commandAttribution = claudeCommand.match(claudeAttributionPattern)?.[0];

if (!skillAttribution) {
  errors.push("Le skill Claude doit porter une ligne d'attribution Claude.");
}

if (!commandAttribution) {
  errors.push(
    "La commande Claude historique doit porter une ligne d'attribution Claude.",
  );
}

if (skillAttribution && commandAttribution && skillAttribution !== commandAttribution) {
  errors.push(
    "Le skill et la commande Claude doivent porter la même ligne d'attribution.",
  );
}

if (workflow.includes("Co-Authored-By:")) {
  errors.push("Le workflow commun doit rester neutre : l'attribution vit dans les adaptateurs.");
}
