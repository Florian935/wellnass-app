...
(function () {
  ...
  function create(canvas, opts) {
    ...
    var RING_IN = 1.95, RING_OUT = 2.17, LAMP_R = 2.06, DAYS = ['L', 'M', 'M', 'J', 'V', 'S', 'D'];
    ...
    function plateDecal(n) {
      return tex(768, function (g, s) {
        ...
        arc('FITTRIO', 0.68, -Math.PI / 2, 1, '800 58px ' + DISPLAY, 6, 'rgba(250,240,244,.9)');
        arc('SÉANCE ' + n, 0.68, Math.PI / 2, -1, '700 40px ' + MONO, 4, 'rgba(250,240,244,.82)');
    ...
    var TH = 0.07;
    var infieldLabel = { big: '25', small: 'KM / SEM' };
    ...
    function refreshInfield() {
      var big, small;
      if (state.mode === 'week' && state.real) { big = String(state.real.kmDone).replace('.', ','); small = 'SUR ' + state.real.km + ' KM'; }
      else { big = String(state.km); small = 'KM / SEM'; }
    ...
    function setValues(v) {
      ...
      dish3.portion = state.kc === 0 ? 1 : state.kc === -250 ? 0.9 : 0.78;
      layoutDish();
      fullNights = state.so >= 8.5 ? 7 : state.so >= 7.5 ? 5 : 4;
      refreshInfield();
    }
    function setPillars() { /* compatibilité : la scène lit les valeurs via setValues */ }
    ...
    canvas.addEventListener('pointerdown', function (e) { down = { x: e.clientX, y: e.clientY, yaw: yawT, pitch: pitchT, moved: false }; });
    window.addEventListener('pointermove', function (e) { if (!down) return; var dx = e.clientX - down.x, dy = e.clientY - down.y; if (Math.abs(dx) + Math.abs(dy) > 6) down.moved = true; yawT = Math.max(-0.9, Math.min(0.9, down.yaw - dx * 0.005)); pitchT = Math.max(-0.25, Math.min(0.3, down.pitch + dy * 0.003)); });
    ...
    var LED_WARM = new THREE.Color(0xffb46b), LED_ALARM = new THREE.Color(0xff4a3a), trackShow = 1;
    ...
    function frame() {
      ...
      var want = Math.max(0, Math.min(5, Math.round(week ? state.real.sessions : state.fq)));
      ...
      trackShow += ((state.km > 0 ? 1 : 0) - trackShow) * k;
      ...
      blobs.nutrition.material.opacity = 0.5;
    ...
    return {
      ...
      setMode: setMode, setReality: setReality, focus: focus,
