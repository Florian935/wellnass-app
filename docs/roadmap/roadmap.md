...
| **Total périmètre de lancement** | **238** | |
...
> **237 → 238 le 10/09/2026** : la ligne **5.40**, refonte UX du pilier Course (CARDIO-UX01).
...
> ⚠️ **La ligne 5.21 reste ✅ — mais elle ne l'était pas jusqu'à aujourd'hui.** Son libellé annonce
> « Mode sans GPS — suivi à la durée seule … couvre aussi le tapis », or `duration_seconds` restait
> `null` sur toute course manuelle : le résumé affichait « Durée — » et « Allure — ». Le statut ✅
> était donc **faux depuis l'origine**, et le test qui couvrait ce chemin simulait le tracker à la
> main (`flushTrack(id, { durationSeconds: 1800 })`) avec un commentaire affirmant que « la durée
> d'une course manuelle est posée par le tracker » — hypothèse fausse en production. Le test passait,
> la fonctionnalité était cassée. CARDIO-UX01 rend le statut exact ; le test est réécrit pour passer
> par le vrai chemin.
