...
| 5.40 | Refonte UX du pilier Course — justesse, hub, course, après-course | Troisième audit de pilier de la semaine, après l'accueil et la musculation. Le mécanisme est **différent** des deux précédents : là-bas, ce qui comptait avait été poussé vers le bas ; ici, **ce qui est calculé n'a jamais reçu de surface** — chaque US ajoutait une ligne de texte à un écran existant plutôt qu'un endroit à elle. **40 constats vérifiés dans le code** (F1 → F40), dont **5 bloquants**. | Moyen | — | 🟢 | 🟡 | **CARDIO-UX01 — audit, maquettes et découpage validés par Florian le 10/09/2026 ; premier lot de code livré le même jour**, en recette → [RECETTES.md](../../RECETTES.md) §59 · [spec](../specs/functional/us/cardio-ux01-refonte-pilier-course.md) · [plan](../plans/cardio-ux01-refonte-pilier-course.md) · [audit](../product/audit-ergonomie-pilier-course.md) (22 pages) · [maquettes](../../design/audit-course/) (11 planches, dont 2 relevés de l'existant). **Traité en un seul lot** sur décision de Florian, là où l'audit proposait dix-sept US — même arbitrage que MUSCU-UX01 la veille. 🟡 **Partiel et c'est assumé** : les **5 constats bloquants sont tous corrigés**, ainsi que le bandeau de segment, le hub, le résumé et le modèle de données. Six chantiers d'ergonomie restent ouverts : listés en fin de section de recette **et** portés au [backlog](../../BACKLOG.md) le 13/09/2026 sous **CARDIO-02 → CARDIO-07** (P1), pour qu'ils survivent à la purge de RECETTES.md. **Ce que ça corrige, et qui n'était pas de l'ergonomie mais de la justesse** : (1) le **chrono affiché n'était pas celui enregistré** — l'écran montrait `now − startedAt`, l'horloge murale, quand la base porte la durée nette hors pauses : 32:10 à l'écran, 29:45 au résumé, et un bouton Pause qui semblait ne rien faire (F9) ; (2) le **mode sans GPS n'enregistrait aucune durée** — `duration_seconds` n'était écrit que par `flushTrack`, appelé seulement par le tracker, lancé seulement en GPS : les 45 minutes du tapis n'étaient **nulle part**, alors que la ligne 5.21 annonce « suivi à la durée seule » et « couvre aussi le tapis » (F15) ; (3) **terminer une course ne cochait pas la séance planifiée** — `markPlannedSessionDone` existait depuis le 12/07 et n'était appelée que depuis le calendrier : le lendemain, le hub proposait de refaire la séance de la veille (F20) ; (4) **Stop clôturait sans confirmation ni retour**, et (5) **aucune course ne pouvait être supprimée** — or une course fantôme entre dans l'ACWR, dans la polarisation, et peut décerner un record 5 km qui met à jour l'allure de référence, donc **toutes** les allures cibles, sans recours (F11, F18). **Deux défauts découverts en implémentant**, tous deux corrigés : le **curseur de phase n'avançait que si le guidage vocal était activé** — or il est désactivé par défaut, donc `run_intervals` restait vide et le tableau « fraction par fraction » de RUN-F4 n'avait rien à afficher pour la majorité des utilisateurs ; et le tableau, lui, n'affichait **qu'une borne** de la plage prévue (`range.min ?? range.max`), faisant lire une plage 4:05–4:10 comme une cible unique (F21). **Le bandeau de segment (F10)** est le gain principal côté ergonomie : toute la machinerie de RUN-F4 — segments typés, rampes, groupes, curseur persisté — était pilotée **à la voix** et n'avait aucune surface visuelle. Écouteurs retirés, le coureur était aveugle au milieu de sa propre séance. ✅ **Migration appliquée sur le cloud le 10/09/2026** (`20260910214329`, `sessions.week_index` + `planned_sessions.adapted_*`, types régénérés) : `ADAPTATION_WRITE_READY` est passé à `true` et le bouton « Appliquer aujourd'hui » est rendu et **écrit en base**. Le drapeau reste en place comme garde-fou — écrire une colonne inconnue du serveur ne casse pas l'écriture locale mais **bloque la file d'upload de toutes les tables**. ⚠️ **La migration a d'abord été refusée** par le CLI (elle était datée du 10/09 à 15:43 alors que NUTRI-UX01 avait déjà poussé jusqu'à 16:22 depuis une branche parallèle) : elle a été **redatée** plutôt que forcée via `--include-all`, qui aurait laissé `schema_migrations` non monotone — exactement la divergence que [MIGRATIONS.md](../../supabase/MIGRATIONS.md) existe pour empêcher. ✅ **Aucune sync rule à redéployer** (`sessions` et `planned_sessions` déjà en `select *`), **aucune dépendance native neuve**. **Vérifié** : typecheck 3 workspaces à 0, lint à 0, **2 753 tests verts**, parité i18n FR/EN contrôlée par `scripts/check-i18n-parity.mjs` (2 386 clés). |
...
| 7.23 | Accueil — la carte « maintenant » | Une carte **épinglée hors grille** en tête d'accueil, qui affiche **un seul sujet** : la prochaine action. Priorité par **table ordonnée** (`NOW_ACTION_ORDER`, patron d'`INSIGHT_ORDER`) : séance/course en cours → planifiée du jour **avec son heure** → repas dû → pesée → check-in du soir → compte rendu → repli. | 🟢 | ✅ | **ACCUEIL-01 — livré le 09/09/2026, recetté et clôturé le 10/09/2026** → [spec](../specs/functional/us/accueil01-carte-maintenant.md) · [plan](../plans/accueil-refonte.md) · [maquettes](../../design/accueil-refonte/) · [RECETTES.md](../../RECETTES.md). Corrige **deux défauts de fond** de l'accueil livré : le pilier était **câblé en dur** sur la musculation (`useTodaySession('strength')`) — un coureur lisait « Rien de prévu aujourd'hui » le jour de sa sortie longue, alors que le widget d'écran d'accueil **Android** balayait bien les deux piliers ; et l'**heure** de la séance n'était affichée nulle part, bien que stockée depuis HORAIRE-01 et présente dans la maquette validée. Rend aussi visibles pour la première fois `useMealDeadline` / `useWeighInDeadline`, qui ne servaient **qu'à programmer des notifications** : l'app savait à l'heure près ce qu'il restait à faire, sans jamais le montrer à l'ouverture. Remplace le widget `today-session`, **promu** en zone épinglée (`{ kind: 'home-pinned' }`) — registre inchangé à 8. Décision pure et testée (19 cas). **Aucune migration ni sync rule** → recettable sur l'APK existant. |
| 7.24 | Accueil — en-tête utile & moment de la journée | Date en clair + **accroche contextuelle** dérivée de la même décision que la carte « maintenant ». Deux fonctions pures : `dayMoment()` (matin/après-midi/soir) et `mealForHour()`. | 🟢 | ✅ | **ACCUEIL-02 — livré le 09/09/2026, recetté et clôturé le 10/09/2026** → [spec](../specs/functional/us/accueil02-entete-utile.md) · [plan](../plans/accueil-refonte.md). Trois défauts corrigés d'un coup : le titre H1 affichait **le nom de l'application** en 28 px extra-bold (information que l'utilisateur possède déjà) sur le pixel le plus visible de l'écran le plus ouvert ; il n'y avait **aucune date** sur un tableau de bord « du jour » ; le salut était **figé** dans le JSON i18n (« Bonjour 👋 » à 22 h). Et un quatrième, le plus gênant au quotidien : le widget nutrition ouvrait le sélecteur d'aliments avec `meal: 'breakfast'` **en dur**, donc le petit-déjeuner à 20 h — le geste le plus fréquent du pilier était systématiquement à reprendre. |
| 7.25 | Accueil — actions rapides | Rangée de **quatre pastilles** fixes sous la carte « maintenant » : Repas (au repas de l'heure) · Course · Pesée · Bien-être. Filtrées par piliers actifs (décision H). | 🟢 | ✅ | **ACCUEIL-03 — livré le 09/09/2026, recetté et clôturé le 10/09/2026** → [spec](../specs/functional/us/accueil03-actions-rapides.md) · [plan](../plans/accueil-refonte.md). Sur les **cinq** gestes réellement quotidiens, **un seul** était à un tap depuis l'accueil : la course imposait un changement d'onglet, la pesée et le check-in bien-être **trois taps** chacun (Muscu › Progression › Mensurations, Réglages › Suivi › Bien-être). Quatre et non cinq : à 392 px de large, une cinquième pastille passerait le libellé sur deux lignes aux grandes tailles de police système. |
| 7.26 | Accueil — densification de la grille (forme `row`, retour du poids) | Fin de `uniformSize` : **formes par défaut différenciées**. Nouvelle forme **`row`** (2 × ½ case) et **résolution verticale doublée** (`small` 1×2, `wide` 2×2, `large` 2×4). Retour du widget **poids**. **Squelettes** de chargement. Régularité **tappable**. | 🟢 | ✅ | **ACCUEIL-04 — livré le 09/09/2026, recetté et clôturé le 10/09/2026** → [spec](../specs/functional/us/accueil04-densification-grille.md) · [plan](../plans/accueil-refonte.md). La plus risquée du lot : touche `sizeSpan`, partagée par les **trois** hubs. Défaut mesuré carte par carte — la case unité étant carrée, une cellule `wide` faisait toujours 352 × 170 quel que soit son contenu : **47 % de remplissage moyen**, et **26 %** pour la carte « vie réelle » hors période (une ligne de texte dans une cellule pleine, onze mois sur douze). ⚠️ **Aucune migration de données** : les dispositions enregistrées se migrent **seules** (`compactVertical` recalcule les lignes dans la nouvelle unité), et c'est **prouvé par 4 tests** — sans eux, le symptôme serait des widgets superposés chez tous ceux qui avaient personnalisé leur accueil. La géométrie de rendu, qui vivait **en double** (`WidgetGrid` / `SortableWidgetGrid`), est extraite en source unique : le passage en demi-cases n'en aurait corrigé qu'une, et le widget se serait posé une demi-cellule trop bas au glisser-déposer. Le poids revient parce qu'il figurait depuis l'origine dans `navigation-ux.md` §3.1 **et** dans la maquette validée. |
| 7.27 | Accueil — pied « la suite » & tirer-pour-rafraîchir | Trois lignes de texte (les prochaines occurrences planifiées, tous piliers) + les liens planning / personnaliser. Premier `RefreshControl` de l'app, branché sur la reprise de synchro PowerSync. | 🟢 | ✅ | **ACCUEIL-05 — livré le 09/09/2026, recetté et clôturé le 10/09/2026** → [spec](../specs/functional/us/accueil05-pied-et-rafraichissement.md) · [plan](../plans/accueil-refonte.md). Du **texte** et non des widgets : répondre par de nouvelles cartes aurait regonflé le registre que 7.21 venait de ramener de 21 à 8. Il n'existait **pas un seul** `RefreshControl` dans toute l'application — le geste est un réflexe, et un écran qui n'y répond pas se lit comme figé ; il force ici la reprise de synchro, utile au retour de connexion. ⚠️ **Écart assumé à la maquette** : la ligne « objectif personnel » n'est pas livrée — un `PersonalGoal` ne porte ni titre ni libellé, la phrase lisible est composée par la carte de `/goals`, et en écrire une seconde version serait la duplication qu'ADR-007 §3 proscrit. À reprendre quand ce libellé sera extrait en brique. |
...
| 7.29 | Dashboards immersifs — une atmosphère par pilier | Les **quatre** écrans d'atterrissage passent à la **scène** : une couleur et une matière par pilier (anneaux qui respirent, silhouette et impact, trace parcourue, niveau qui monte), un **moment** par situation, et le repli au défilement. Plus « l'app qui comprend » : « Pourquoi ? » sur les chiffres calculés, brief du matin lu à voix haute, « Et si… » déterministe, et un **assistant IA désactivé par défaut** (photo de repas, reformulation). | 🟢 | ✅ | **DASH-01 — code livré le 13/09/2026, en recette** → [spec](../specs/functional/us/dash01-dashboards-immersifs.md) · [plan](../plans/dash01-dashboards-immersifs.md) · [maquettes](../../design/dash-immersifs-2026-09/) (19 planches) · [RECETTES.md](../../RECETTES.md) §62 (38 critères). ✅ et non 🟡 : le code est **complet**, même convention que 7.20 et 7.21 — le 🟡 signalerait un socle incomplet. **Une seule vague**, sur demande explicite de Florian le 13/09/2026 (« fait TOUT d'un seul coup »), pour une recette finale unique. ⚠️ **Une migration poussée** (`ai_consent_at` + `ai_usage`) mais **aucune sync rule à déployer** : `user_settings` est lue en `select *` et `ai_usage` n'est volontairement pas publiée — le réflexe « migration ⇒ sync rule » ne vaut que pour une table **synchronisée**. 🔴 **La surface IA a été RETIRÉE du build de lancement le 13/09/2026**, sur décision de Florian : l'app est gratuite en V1, et l'[analyse du 15/07/2026](../product/ia-integration-analyse.md) plaçait l'IA en **palier payant post-V1** (« le prix du palier doit couvrir le coût IA du user le plus actif »). La livrer gratuite revenait à payer le modèle pour tout le monde. La plomberie reste dormante (migration appliquée, fonction Edge dans le dépôt, aucun secret posé → **aucun coût**) ; « Demande-moi » est conservée en version **déterministe**, ce que l'inversion d'origine rendait possible sans rien perdre. ✅ **Play Store : plus rien à déclarer** côté envoi de données à un tiers. ⚠️ **Aucun chiffre affiché ne sort d'un modèle** (R7) : la photo rend des aliments et des grammes, le catalogue local calcule les calories ; « Demande-moi » calcule la réponse côté client et le modèle ne fait que la formuler — sans IA, les mêmes réponses s'affichent. ⚠️ **Écarts assumés**, listés en fin de §62 : partage de séance laissé sur l'écran de bilan, écart de prédiction 10 km non calculable (pas d'historique de records), compte à rebours à l'heure et non à la minute. |
| 7.30 | Le Labo — là où tes piliers se croisent | Un **onglet dédié** où l'utilisateur voit sa semaine réelle tous piliers confondus, **dose ses leviers ensemble** avec l'effet chiffré, **enquête** quand une courbe cale, et garde **ce que le Labo a appris de lui**. Scène 3D dès le premier écran (disques de fonte, piste, assiette, lampes-nuits), repli 2D. Toute écriture passe par une feuille « ce qui change dans ton plan ». | 🟢 | ✅ | **LABO-01 — code livré le 15/09/2026, en recette** → [spec](../specs/functional/us/labo01-labo.md) · [plan](../plans/labo01-labo.md) · [analyse](../product/analyse-labo-2026-09.md) · [maquettes + prototype jouable](../../design/labo-2026-09/) · [ADR-008](../adr/ADR-008-scene-3d-composant-dom.md) · [RECETTES.md](../../RECETTES.md) §66 (51 critères). **Une seule vague**, sur demande explicite de Florian le 15/09/2026 (« on va tout coder ICI d'un seul coup sec, d'une seule vague et avant le Play Store »), après quatre allers-retours de design validés à l'écran — la maquette était **jouable**, c'est elle qui a servi de spécification visuelle. 🔴 **La question posée n'était pas « est-ce beau » mais « pourquoi y revenir »** : « il faut qu'il y ait un intérêt à venir dans le labo, pas que ce soit uniquement un joujou ». D'où la règle qui structure tout l'écran : **tout part de la semaine en cours et se termine par un geste qui écrit dans le plan** — et **rien ne s'écrit sans la feuille** de confirmation, qui nomme le changement, le pilier et les écrans où ça se verra. Une proposition qui **n'écrit rien** (elle ouvre un écran) part au contraire tout de suite : lui imposer une feuille apprendrait à confirmer sans lire. ⚠️ **Aucun chiffre ne sort d'ailleurs que d'un calcul déjà recetté** : le prototype affichait « 10 km −25 s », **coefficient inventé, non livré** — l'écran **n'affiche aucune projection d'allure ni de chrono**, et le dit en toutes lettres. Ce qui s'affiche vient de DASH-01 (« Et si… »), MUSCPWR-01, META-19, MN-02, MN-06, FUEL-01, GARDE-01, COLLIS-01, CARDIO-UX01. **Aucune analyse nouvelle n'est calculée**, comme INSIGHTS-01 avant lui : le Labo est la **maison** des 28 analyses inter-piliers déjà livrées, qui n'en avaient aucune. ⚠️ **Le sommeil est saisi, pas capté** : Health Connect « sommeil » restant écarté, la nuit devient une **note facultative du check-in** BIEN-01 (colonne nullable, pas d'un quart d'heure) → **déclaration Play « Health apps » inchangée à 6 types, aucun délai externe ajouté**. ⚠️ **Aucune expérience sur les calories** : quand le poids stagne, le Labo propose de **mesurer** (saisir aussi les week-ends), jamais de restreindre. Une expérience dure 4 semaines dont l'ordre est **tiré au sort à l'écriture, une fois**, et son **verdict reste scellé** jusqu'au bout — voir un résultat partiel changerait le comportement pendant la mesure ; l'adhérence est mesurée **sans saisie supplémentaire**. ✅ **2 migrations poussées le 16/09/2026** (`daily_wellbeing.sleep_minutes` + table `lab_experiments` avec index unique partiel, puis publication) et ✅ **sync rule déployée** (table neuve — étape déjà oubliée sur BIEN-01, VIE-01 et RUN-F2c), `LAB_WRITE_READY` passé à `true` : **la recette peut démarrer**. ⚠️ `--include-all` a été nécessaire, ces migrations étant horodatées **avant** celles de DEPENSE-01 déjà appliquées — sans conséquence, les deux jeux sont strictement additifs et disjoints. 🔴 **Nouvelle décision d'architecture, [ADR-008](../adr/ADR-008-scene-3d-composant-dom.md)** : la scène est un **composant DOM Expo** (`'use dom'`) et non une réécriture `expo-gl` / react-three-fiber — `expo-gl` n'expose pas de canvas 2D, donc **toutes** les textures dessinées et la post-production auraient été à refaire, pour rendre *moins* que ce qui était déjà validé. Coût tenu : l'état qui franchit le pont est **sérialisable et testé comme tel**, la scène est de **hauteur fixe** (une WebView dans une liste qui défile vole les gestes), elle décide **elle-même** de son repli 2D si WebGL manque, et l'app lui **pousse tous ses textes** (aucune chaîne en dur, FR+EN). ⚠️ **Écarts assumés** : pas de « Croiser » libre (l'enquête part d'une question réelle plutôt que d'un tableau vide), pas de bilan de cycle, et les **séances de musculation se dosent mais ne s'écrivent pas** — elles viennent du programme, et les écrire ici les ferait diverger en silence de la séance réellement proposée. **Vérifié** : typecheck 3 workspaces à 0, lint à 0, **6 795 tests verts** (587 admin + 3 220 mobile + 2 988 shared, sur `dev` avec les chantiers voisins), bundle Android produit **avec la scène DOM embarquée** (`www.bundle/*.html` + le JS de three). 🔴 **Un garde-fou a attrapé un vrai oubli** : le test de complétude de l'export RGPD a refusé `lab_experiments` — sans lui, l'archive aurait contenu les mesures d'une expérience **sans son protocole**, donc des chiffres ininterprétables. ⚠️ Le bundling **web** échoue sur `better-sqlite3` **avant cette US et indépendamment d'elle** (rendu statique qui tire le build Node d'`op-sqlite`). 🔴 **La revue de code avant commit a sorti 3 défauts BLOQUANTS**, tous corrigés et figés par des tests : une expérience terminée restait `running` à vie (modèle plus jamais relançable, et rejet d'upload qui aurait figé toute la file PowerSync) ; le widget « énergie » de l'accueil **effaçait la nuit** qui venait d'être saisie (`patch` réécrivait toutes les colonnes — défaut antérieur à l'US, dont elle élargissait la portée) ; et il manquait le garde-fou d'écriture avant migration (`LAB_WRITE_READY`, patron CARDIO-UX01) alors que le champ vit dans un écran **partagé**. Plus cinq importants : crash possible de l'écran sur six pesées le même jour, onglet qui **ne disparaissait jamais** sans pilier actif (R9), feuille qui promettait une écriture de protéines non faite sans poids connu, absence totale de gestion d'erreur sur les gestes qui écrivent (leçon CONF-06), et une assiette qui **rétrécissait** pour une prise de masse. ⚠️ **Cinq constats restent ouverts**, listés au CHANGELOG — dont le plus gênant : la fenêtre glissante de 56 jours fait qu'**un acquis peut se désapprendre** 28 jours après son verdict. |
| 7.31 | Labo IA — évaluer un modèle sur des données factices | On avait **chiffré** l'IA ([analyse du 15/07/2026](../product/ia-integration-analyse.md) : coûts, architecture, phasage) sans jamais **voir** ce qu'un modèle rend sur nos données — et le retrait de la surface IA le 13/09 avait emporté le seul moyen de l'évaluer. Un **labo** rouvre ce chemin à coût nul : fournisseur **gratuit** (Gemini), **données factices**, et un jeu de test qui plante **six signaux connus** pour qu'on puisse **noter** les réponses au lieu de les admirer. | 🟢 | ✅ | **IA-LAB-01 — code livré le 15/09/2026, en recette** → [spec](../specs/functional/us/ialab01-labo-ia.md) · [plan](../plans/ialab01-labo-ia.md) · [RECETTES.md](../../RECETTES.md) §67 (40 critères). Pas de maquette : surface de test interne, **hors build de lancement** (seul chemin : Réglages → Labo IA, opt-in éteint par défaut). ✅ **Aucune migration, aucune sync rule, aucun nouveau build** — `ai_consent_at` et `ai_usage` datent de DASH-01, aucune dépendance native. ⚠️ **Trois gestes humains** avant recette : clé Gemini gratuite, `supabase functions deploy ai-assist`, et le script [`ia-purge-et-dataset.sql`](../../supabase/scripts/ia-purge-et-dataset.sql). Sans clé posée, la fonction répond `ai_unavailable` et **rien n'est facturé**. 🔴 **Données factices obligatoires** : le palier gratuit de Gemini peut utiliser les requêtes pour entraîner ses modèles (analyse §7.2) — c'est la raison d'être du script de remise à plat. 🔴 **Exception assumée à « aucun chiffre affiché ne sort d'un modèle »** : le labo affiche la réponse **brute**, et c'est tout son objet. Elle ne peut donc jamais être réutilisée par un écran produit. La minimisation est tenue par une **liste blanche testée** (`AiSnapshot`) : ni identité, ni date de naissance, ni texte libre, ni trace GPS. |
| 7.32 | Lettre à ton futur toi — un mot scellé avec l'objectif | OBJ-01 pose une cible et un anneau, mais ne garde **aucune trace du pourquoi** : trois mois plus tard, l'utilisateur voit « 68 % » sans se rappeler ce qui l'avait décidé. En créant un objectif, il peut écrire quelques lignes à son futur lui-même. Le mot est **scellé** : il ne se rouvre que dans trois cas — échéance passée, objectif atteint, ou suppression. | Moyenne | ✅ | **LETTRE-01, 18/09/2026** — troisième et dernière US du lot 1 de la salve « carnet d'innovation » (idée 34), et la seule qui joue sur la **rétention émotionnelle** plutôt que sur la donnée. Trois colonnes additives sur `personal_goals` (migration `20260918182701` poussée le jour même) ; ✅ aucune sync rule à redéployer — la table était déjà publiée — mais les colonnes sont bien déclarées dans `powersync/schema.ts`, la panne silencieuse de CYCLE-01 puis FANT-01. 🔴 **Correction constatée à l'implémentation** : la spec s'appuyait sur une notification d'échéance « déjà en place » côté OBJ-01 — **elle n'existe pas**, OBJ-01 l'avait explicitement écartée (sa décision D4). Le déclencheur devient **in-app** (la carte de l'objectif terminé propose de relire) ; **aucune notification n'a été ajoutée**, donc aucune permission nouvelle avant la soumission Play. ⚠️ **Pas d'IA, jamais** (R7) : le texte n'est ni analysé ni reformulé ni envoyé à un tiers. ✅ **Export RGPD couvert sans rien câbler** : l'export de CONF-01 lit `SELECT *`, donc la lettre en fait partie dès sa création. ⚠️ Étape « design » sautée (surfaces existantes), comme FANT-01. En recette → [RECETTES.md](../../RECETTES.md) §73 · [spec](../specs/functional/us/lettre01-lettre-futur-moi.md) · [plan](../plans/lettre01-lettre-futur-moi.md) · [analyse](../product/analyse-innovation-2026-09.md). |
| 7.33 | L'IA raconte le dossier d'enquête | Le Labo sait déjà dire pourquoi une courbe cale : plateau détecté, suspects classés par force, facteurs écartés, expérience qui tranche. Il faut le lire en entier pour comprendre. Un bouton « Résumer » en fait deux phrases — et **tout chiffre du résumé absent du dossier fait jeter le résumé**. | Moyenne | ✅ | **NARR-01, 19/09/2026** — première US du **lot IA** de la salve « carnet d'innovation » (idée 5). 🔴 **L'inventaire du 19/09 a montré que « L'Enquête » était déjà livrée** par LABO-01 (`lab-investigations.ts` + onglet « Pourquoi ? ») et que le socle IA était posé par IA-LAB-01 : l'US se réduit donc à ce qui manquait vraiment, **la première lecture**. Même constat qu'avec l'idée 9, déjà couverte par DASH-01 — la salve d'idéation surestimait deux fois ce qui restait à construire. ✅ **Aucune migration, aucune route serveur, aucun secret** : le type `coach` de la fonction Edge suffit. ⚠️ **Opt-in strict** : sans consentement IA, le panneau est exactement celui d'avant (R3). 🔴 **Le garde-fou est mécanique, pas contractuel** (`ai-narration.ts`, 20 tests) : l'invite système demandait déjà au modèle de ne rien inventer, cette US **vérifie qu'il a obéi** et jette le résumé entier au premier nombre inconnu — sans réessayer, pour que la fréquence du défaut reste visible. ⚠️ **Portée assumée** : le garde-fou porte sur les **nombres**, pas sur le sens ; une phrase fausse sans chiffre passerait. Écrit dans la spec (D4) et à l'écran. En recette → [RECETTES.md](../../RECETTES.md) §74 · [spec](../specs/functional/us/narr01-narration-dossier.md) · [plan](../plans/narr01-narration-dossier.md). |
| 7.34 | Le Conseil des trois — les deux issues d'une contradiction, chiffrées | GUID-01 dit que « prise de masse » et « sèche » se contredisent, et propose deux issues de poids égal — **sans aucun chiffre**. Le Conseil les chiffre : calories/jour, poids à 8 semaines, protéines, force avec sa fourchette, et les tensions que chaque voie crée. | Moyenne | ✅ | **CONS-01, 19/09/2026** — seconde et dernière US du **lot IA** (idée 6). 🔴 **Le « moteur de compromis qui n'existe pas »** — mots de `goal-conflicts.ts` en écrivant la règle — **existait déjà** : LABO-01 l'avait construit sous le nom de `lab-composer`. L'US ne fait que tendre le fil entre les deux, et **n'ajoute aucun coefficient**. ✅ **Aucune écriture nouvelle** : les deux boutons sont ceux de GUID-01, avec les mêmes conséquences — un écran d'arbitrage qui déclencherait autre chose que ce qu'il chiffre serait pire que pas d'écran. ⚠️ **Une règle sur deux** : `enduranceVsMass` n'est pas chiffrée, faute de RN-17 (volume de course × déficit, « non construit » au catalogue) — la carte reste alors celle de GUID-01, sans lien. ⚠️ **Aucune projection de chrono**, contrairement à la maquette du 13/09 qui affichait « 19:35–20:10 » : aucun calcul validé ne relie une dose à un temps de course, et le dépôt avait déjà refusé ça le 15/09. L'écran le dit à l'utilisateur. ✅ Le résumé IA de NARR-01 est réutilisé tel quel, garde-fou compris. En recette → [RECETTES.md](../../RECETTES.md) §75 · [spec](../specs/functional/us/cons01-conseil-des-trois.md). |
| 1.30 | Objectif utile & régime de guidage | L'objectif principal, demandé à l'onboarding depuis le premier jour, ne pilotait **rien** : `main_goal` était lu à **8 endroits**, tous le même appel `objectiveFromGoal(...)` (le repli de l'objectif nutritionnel), et `performance` / `health` tombaient dans le même `default` — **deux options sur quatre** donnaient une application identique à celle de quelqu'un ayant appuyé sur « Passer ». Cette US livre le fil manquant entre l'objectif et les moteurs déjà construits, et ajoute le **régime de guidage** (guidé / accompagné / autonome, par pilier). | 🟢 | ✅ | **GUID-01 — analyse et maquettes validées par Florian le 13/09/2026, code livré le même jour**, en recette → [spec](../specs/functional/us/guid01-objectif-regime-guidage.md) · [plan](../plans/guid01-objectif-regime-guidage.md) · [analyse](../product/analyse-objectif-guidage-2026-09.md) (13 p.) · [maquettes](../../design/objectif-guidage-2026-09/) (3 planches) · [RECETTES.md](../../RECETTES.md) §64 (32 critères). **Traité en un seul lot** sur décision de Florian, là où l'analyse en proposait cinq. 🔴 **L'axe du guidage est « qui décide », pas « combien de messages »** : graduer le volume d'interventions aurait fabriqué un mode « l'app me harcèle », coupé en dix jours. Les trois régimes envoient autant de messages ; `guided` **annonce des décisions prises** (et offre d'annuler), `assisted` **pose des questions**, `autonomous` n'en a plus à poser. **Trois signaux de sécurité franchissent tous les régimes** (ACWR critique, douleur répétée, déficit sévère) et ne sont **jamais** appliqués d'office — on alerte, on ne décide pas à la place de quelqu'un sur un sujet de santé. **L'étape 4 de l'onboarding change de question sans changer de place** : « Niveau d'affichage » était un réglage d'interface déguisé en question de profil, et c'est faute de mieux que la muscu s'en servait comme **proxy d'expérience** pour trier ses programmes (« je veux peu d'infos » = « je débute ») — un confirmé qui aimait les écrans épurés recevait des programmes débutants. L'affichage se **déduit** désormais du régime, et le niveau d'entraînement a enfin sa colonne, demandée **devant la bibliothèque** et non à l'onboarding (décision F intacte : le parcours garde 5 étapes). **Le pilier Musculation gagne un écran de profil** — il n'en avait aucun, alors que nutrition et course en ont un depuis toujours. **La carte de contradiction** (masse ↔ sèche, masse ↔ marathon) est le plus petit morceau démontrable de la décision H : jusqu'ici les quatre notions d'objectif du dépôt pouvaient se contredire **en silence**, sur des écrans différents. Explicable et contestable (IDEAS 25/07/2026). ⚠️ **Écarts assumés** : la **génération** d'un plan hebdomadaire sur mesure n'est pas livrée (le régime guidé **sélectionne** un programme éditorial, le duplique et l'active — composer des séances reviendrait à inventer des données d'entraînement, ce que CONTENU-01 a refusé) ; `carbTarget` est **déclaré et testé mais non câblé** (le bonus des jours d'entraînement est déjà automatique et piloté par un réglage utilisateur — le passer sous le régime aurait été une régression pour le mode par défaut) ; **2 règles de contradiction sur 3** (RN-17 n'existe pas comme calcul livré). 🔴 **Corrigé en revue (13/09/2026), et ce n'était pas mineur** : sans garde-fou, l'écriture d'une de ces colonnes mettait en file une op PowerSync que le cloud rejette — et comme la file est **sérialisée**, c'est la remontée de **toutes** les tables qui se figeait, pas seulement celle-ci. Un drapeau `GUIDANCE_WRITE_READY` (patron `ADAPTATION_WRITE_READY` de CARDIO-UX01) retire les 8 colonnes de l'écriture tant que la migration n'est pas sur le cloud ; **à passer à `true` dans le même geste que `db:push`**. Trois autres constats corrigés : l'annulation d'un déplacement automatique était **défaite** au premier aller-retour sur l'écran (état local → store), deux collisions traitées dans la même passe pouvaient atterrir **le même jour** (un conflit par passe), et le sélecteur de régime du profil Musculation **décrivait trois comportements inexistants** — il est retiré jusqu'à ce que les moteurs muscu lisent le régime. ✅ **Migration poussée le 13/09/2026** (8 colonnes additives nullable), drapeau `GUIDANCE_WRITE_READY` à `true`, aucune sync rule à déployer (`profiles` est publiée en `select *`) — **la recette peut démarrer**. ⚠️ Le push a d'abord été refusé : le cloud portait `20260913182920` (seed CIQUAL v2) dont le fichier n'était pas encore commité, débloqué le jour même par `89396aa0`. **Vérifié** : typecheck 3 workspaces à 0, lint à 0, **5 968 tests verts** (3 121 Jest + 2 847 Vitest), **0 warning de lint**, dont un garde-fou neuf sur les trois gestes d'ajout de colonne au profil et un test de **parité FR ↔ EN** qui n'existait pas. |
...
| 3.59 | Refonte UX du pilier Musculation — hub, séance, après-séance, entrée programme | Second audit du pilier, deux mois après celui de juillet. Les cinq problèmes de flux avaient été corrigés ; ce qui restait tenait à **l'accumulation** — deux mois de fonctionnalités ajoutées une à une sur des écrans jamais rehiérarchisés. **17 constats vérifiés dans le code**, un mécanisme unique : *ce qui compte le plus a été poussé vers le bas par ce qui compte moins*. | 🟢 | ✅ | **MUSCU-UX01 — audit et maquettes validés par Florian le 10/09/2026, code livré le même jour**, en recette → [RECETTES.md](../../RECETTES.md) §57 (59 critères) · [spec](../specs/functional/us/muscu-ux01-refonte-pilier-musculation.md) · [plan](../plans/muscu-ux01-refonte-pilier-musculation.md) · [audit](../refonte-muscu/audit-ux-2026-09.md) · [maquettes](../../design/refonte-muscu-2026-09/) (19 planches avant/après). **Traité en un seul lot** sur décision de Florian, là où l'audit en proposait six. **4 arbitrages du 09/09/2026** : (1) tout le pilier, Progression et Historique compris ; (2) **barre d'action collante** en séance plutôt qu'un tiroir ou un pavé maison ; (3) les **trois niveaux d'affichage conservés**, mais exposés depuis la séance ; (4) **plafond testé** sur le hub + masquage des tuiles vides. **Ce que ça corrige, mesurable** : le hub passait de 2,4 écrans de scroll (7 tuiles rendues **même vides**, faute du prédicat `isActive` que l'accueil passait déjà) à un écran ; « Valider la série » — répété 30 à 40 fois par séance — quittait le bas d'une carte de ~560 px, où le clavier le recouvrait (`adjustResize`, aucun `KeyboardAvoidingView`) ; un programme **ne pouvait jamais commencer aujourd'hui** (`weekStart` initialisé au lundi suivant) et exigeait d'affecter chaque séance à un jour sans aucune suggestion. **Trois promesses de spec enfin tenues** : le retour haptique à la validation (navigation-ux §4.2, jamais implémentée côté muscu — le planning et le running en avaient), la durée estimée avant de démarrer (musculation §4.2), la suppression d'une séance passée (§6.1). Et **MUSC-F15**, qui calculait l'adhérence depuis le 01/08 sans jamais l'afficher, devient la barre « semaine 3 sur 8 ». **Plafond `MAX_STRENGTH_WIDGETS = 3`**, appliqué par un test sur le modèle de `MAX_HOME_WIDGETS` : le hub muscu n'en avait aucun, et c'est exactement pour ça qu'il est passé de 4 à 7 widgets pendant que l'accueil se dégonflait de 21 à 7. Les 4 retirés ont chacun une destination (barre de progression, ligne d'annuaire, `/progress` › Vue d'ensemble). ⚠️ `strength-templates` avait été créé par **Refonte-D** comme point d'entrée permanent : le déplacer est un arbitrage assumé, pas un oubli. **Progression** repasse de **8 sections empilées** à **3 onglets** — le code admettait lui-même « seuil de repli ADR-007 atteint, accepté tel quel », trois sections plus tôt. Les sections sont **réparties, pas réécrites** : aucun calcul touché. ✅ **Aucune migration, aucune sync rule, aucune dépendance native** (`expo-haptics` était déjà là) → recettable sur un build de la branche. ⚠️ `generatePlannedSessions` change de comportement : les occurrences **antérieures à la date de début** ne sont plus générées, donc la première semaine est partielle. **Non-régression course vérifiée** : côté course `startDate` reste un lundi, donc rien n'est filtré. 🔴 **Développée dans un worktree isolé**, en parallèle de `feature/accueil-refonte` (autre session) : zones de contact au merge sur `packages/shared/src/widgets.ts` (régions distinctes : `STRENGTH_*` contre `HOME_*`) — `WidgetGrid` n'est **pas** touché, `isActive` y existait déjà. **Vérifié** : typecheck 3 workspaces à 0, lint à 0, **2 708 tests verts**, parité i18n FR/EN contrôlée par un script neuf (`scripts/check-i18n-parity.mjs`, 2 245 clés). 🔁 **3ᵉ passe de recette le 19/09/2026 : 3 causes racines corrigées** (MUSCU-FIX01, `dev`) — l'« écran noir » de la séance libre (`/workout` ne lisait pas `isLoading` et annonçait « Aucune séance en cours » **sur la séance qu'il venait de créer**), la **séance vide sans barre d'action** (une séance libre démarre toujours à zéro exercice : son écran d'arrivée était un cul-de-sac), et les **templates devenus inatteignables** — `/templates` n'avait plus qu'un point d'entrée, qui ne s'affiche qu'à partir d'un template, alors que c'est le seul écran pour en créer un. 🔴 Cette dernière est une **régression de cette US** : le retrait du widget `strength-templates` désignait « la ligne d'annuaire *Exercices, programmes, templates* » comme repli, ligne qui porte bien ce libellé mais n'ouvrait que les exercices — **la destination promise n'a jamais été construite**. Silhouette de la scène redessinée au passage. [spec](../specs/functional/us/muscu-fix01-flux-seance.md) · [RECETTES.md](../../RECETTES.md) §76. 🔁 **1ʳᵉ passe de recette le 11/09/2026 : 7 constats corrigés** (`fix/muscu-ux01-recette-passe-1`) — dont **deux requêtes SQL invalides** qui rendaient inatteignables la carte « séance du jour » du hub (`e.name` sur `exercises`, table sans colonne `name`) et **tous** les écarts du résumé (`w2.owner_id` sur `workouts`, qui porte `user_id`) : sous PowerSync, `useQuery` avale l'erreur et l'écran affiche son état « pas de donnée », qui est légitime — aucun crash, aucun test rouge. Un sweep prépare désormais les **162** requêtes des repositories contre le schéma local. Aussi : bande de stats qui débordait, clé i18n brute `workout.displayLevel.title`, haptique muet (`performAndroidHapticsAsync` ignoré par Android quand la vibration au toucher est coupée — retour à l'API `Vibrator`, ce qui répare au passage la vibration de fin de repos), et **module force inatteignable depuis MUSCPWR-01** (ses deux seules entrées vivaient dans la section qui se masquait). Statut inchangé : **toujours en recette**, critères 60 à 67 ajoutés. 🔁 **2ᵉ passe le 11/09 : 6 critères sur 8 validés, 3 constats de plus** — le clavier recouvrait la barre de saisie (§57.19), défaut *de cette US* jamais corrigé : depuis Expo SDK 54 l'edge-to-edge est **forcé** sur Android et `adjustResize` ne redimensionne plus la fenêtre, donc un montage en colonne ne suffit plus (hook `useKeyboardHeight`) ; l'haptique restait muet pour une **seconde cause** — `expo-haptics` impose une amplitude de 30/255, sous le seuil de perception d'un Pixel 6a, d'où le retour à `Vibration` de React Native qui laisse l'amplitude par défaut du constructeur ; et la bande de stats a été reprise (unité séparée du nombre, tonnage arrondi) pour que les quatre chiffres aient la même taille. Critères 68 à 73. |
| 3.60 | Langage de mouvement — une physique par pilier, socle d'animation partage | L'app etait **immobile** : `react-native-reanimated` 4.5 et `react-native-worklets` etaient installes mais utilises par **2 fichiers sur ~80 ecrans** (le glisser-deposer du planning), `CelebrationCard` etait le **seul** composant reellement anime, `RingGauge` calculait son arc une fois (l'anneau apparaissait deja rempli), le timer de repos etait **un texte qui decremente**, et les 3 fonctions haptiques — ecrites et documentees sur 30 lignes — n'etaient appelees que depuis 2 fichiers, tous deux en muscu (aucun retour tactile en course ni en nutrition). Surtout : `design/design-system.md` § « Animations cles » nommait **5 animations validees en maquette** (`prpop`, `sheetup`, `pulsedot`, `dashmove`, `fadeslide`) dont **aucune** n'existait dans le code. | 🟢 | 🟡 | **MOTION-01 — analyse et maquettes validees par Florian le 12/09/2026, code livre le meme jour**, en recette → [RECETTES.md](../../RECETTES.md) §61 (24 criteres) · [spec](../specs/functional/us/motion01-langage-mouvement.md) · [plan](../plans/motion01-langage-mouvement.md) · [maquettes](../../design/motion-01/) (6 planches **animees** — un brief sur le mouvement ne se juge pas sur des images fixes). **Traite en un seul lot** sur decision de Florian, la ou l'analyse proposait cinq lots successifs. 🟡 **Partiel et assume : 19 effets livres sur les 45 analyses.** Le socle est complet (jetons `theme/motion.ts`, `useAppReducedMotion`, `PressableScale`, `AnimatedNumber`, `AnimatedRing`, `AnimatedBar`, `StaggerIn`, `Breathe`) ; ce qui reste n'est que du branchement. **4 effets sont ecartes pour une raison de fond**, listee en §60 : la pastille d'onglet glissante (reconstruction de la barre `expo-router`, qui porte deja le masquage des piliers de la decision H), `dashmove` (propriete de **style** MapLibre, non animable sans rafraichissement JS par image), le toast de record **en seance** (les records ne sont evalues qu'a la cloture — c'est du travail de donnees), et la transition seance → resume (`workout-summary.tsx` est reecrit en parallele par MUSCU-UX02). **Ce que ca corrige, concretement** : `Button` ne produisait qu'un `opacity: 0.85`, invisible sous le pouce qui recouvre le bouton ; le repos — l'ecran ou l'on reste le plus longtemps immobile — n'avait aucune representation visuelle du temps restant ; le bilan calorique repartait de zero a chaque ajout d'aliment, cachant precisement la portion ajoutee. **Une physique par pilier** (suggestion de Florian) : impact pour la muscu (spring 420/18), flux pour la course (lineaire, boucles), remplissage pour la nutrition (**overshoot zero** — une jauge de calories qui depasse affiche un chiffre faux), souffle pour l'accueil. **Accessibilite** : `navigation-ux.md` §4.2 exigeait « animation + son (desactivable) » depuis le cadrage — l'interrupteur « Animations » des Reglages le livre enfin, et se combine au reglage systeme par un **OU** (l'app ne peut pas contredire Android). `CelebrationCard` etait le **seul** fichier a gerer « reduire les animations » ; c'est desormais un hook partage, synchrone, que lisent les 8 primitives. Les vibrations, elles, **survivent** a la coupure. **3 pieges rencontres** : (1) le mock Reanimated de la bibliotheque est inutilisable ici — il reimporte `src/index.ts` et declenche le chargement natif qu'il devait eviter, d'ou un mock ecrit a la main dans `jest.setup.ts` (sans lui, **26 suites** mouraient a l'import des que Reanimated entrait dans une primitive partagee) ; (2) `Intl` **n'existe pas** sur le thread UI, donc `AnimatedNumber` formatait en francais en dur — les separateurs sont devenus des parametres tires de la langue, sans quoi regression i18n dans une app bilingue depuis le jour 1 (decision G) ; (3) `useFocusEffect` et `useIsFocused` **levent** hors conteneur de navigation — un halo decoratif n'a pas a faire planter son ecran, les boucles infinies s'annulent donc sur `AppState`. ✅ **Aucune migration, aucune sync rule, aucune dependance native** (tout etait deja au `package.json`) → recettable sur un build de la branche, sans repasser par EAS. 🔴 **Developpee dans un worktree isole**, en parallele de `feature/muscu-ux02-bilan-seance` (~1800 lignes non commitees sur l'ecran recap) : aucun fichier commun, `CelebrationCard` est upgrade **sans toucher** aux deux ecrans de resume qui en heritent. **Verifie** : typecheck 3 workspaces a 0, lint a 0, **170 suites / 2862 tests verts** (169 / 2856 avant). |
| 3.61 | Mode immersif de la séance — la séance vivante, en plus du mode classique | La séance est **juste** (MUSCU-UX01) et **animée** (MOTION-01), mais **muette** : la série moyenne et le record reçoivent la même réponse, les records ne sont connus qu'à la clôture, l'effort est un temps mort pour l'app, et le repos — la moitié de la séance — est le moment où l'on part sur une autre app. Un **second mode d'affichage** ajoute la barre chargée, l'effort au tempo, le cadran de reps, le repos qui respire, le fantôme de la dernière séance, le corps qui chauffe, un coach vocal à gabarits et le fil hors de l'app. | 🟢 | ✅ | **MUSCU-UX03 — livré le 13/09/2026, en attente de recette** → [spec](../specs/functional/us/muscu-ux03-mode-immersif.md) · [plan](../plans/muscu-ux03-mode-immersif.md) · [maquettes](../../design/muscu-ux03-mode-immersif/) (2 toiles, dont un **prototype jouable** avec voix, sons et vibrations). 🔴 **Décision de Florian (13/09/2026) : l'immersif est un mode EN PLUS, le classique reste inchangé** — certains utilisateurs veulent « leur série, point barre ». Le classique ne gagne qu'une **pastille discrète de record**. ⚠️ **À acter** : le fantôme est une comparaison avec soi-même, sans points ni badges — compatible avec la **décision C** (gamification hors V1), mais à confirmer ; et l'**abonnement** (3 options maquettées, recommandation : immersif gratuit, coach IA payant) reste ouvert, sans effet sur cette US (V1 gratuite, décision D). ✅ **Aucune migration, aucune sync rule, aucune dépendance native** — préférences locales (`secureStorage`), `expo-speech` et `expo-notifications` déjà au projet → recettable sur un build de la branche. ⚠️ **Hors périmètre assumé** : le coach **IA** (proxy serveur, consentement, palier IA — analyse IA §5 à §7), les **sons** (dépendance native), le stockage du temps sous tension (migration), le décompte à la seconde dans la notification (module natif). 🔴 Développée dans un **worktree isolé** (`.claude/worktrees/muscu-immersif`), en parallèle de `dash01`, `nutri-refonte` et `corps02`. |
| 3.62 | Identité d'un pilier tenue par toute la page — accent, surfaces, coulée de la scène | La scène d'un pilier était en couleur pendant que **toutes les cartes en dessous** restaient brunes avec un accent terracotta : **trois identités sur un seul écran** (scène bordeaux `#6b0028`, onglet rose `#e07a98`, cartes brunes `#30271e`). Les surfaces prennent désormais la teinte du pilier affiché, l'accent devient celui du pilier, et la scène **coule** sur le haut de la page au lieu de s'arrêter net. | Élevée | ✅ | **MUSCU-UX04 — livrée le 19/09/2026** sur demande de Florian en recette (« faut que tout soit ISO »), en recette → [spec](../specs/functional/us/muscu-ux04-identite-pilier.md) · [RECETTES.md](../../RECETTES.md) §77. 🔴 **Teinte à luminance CONSTANTE**, et ce n'est pas un raffinement : un mélange ordinaire assombrit les surfaces, or la palette claire est à **4,53-4,55** pour un seuil de 4,5 — mesuré, un mélange à 10 % vers le bordeaux faisait tomber **six paires** sous le seuil, dans un thème que CONF-07 venait de rendre conforme. À luminance constante, tout rapport de contraste est conservé par construction. 🔴 **Bug d'accessibilité corrigé au passage** : le réglage « Couleurs des menus » posait `#6b0028` comme accent muscu — **1,15:1** sur une carte sombre ; un interrupteur de réglages rendait libellés et icônes illisibles. Défauts réalignés **et** toute couleur ramenée au seuil par `readableOn` avant usage. 🎨 **Bordeaux adouci le 19/09/2026**, sur retour de Florian (« pas très smooth, pas très sexy ») : les trois arrêts d'origine étaient à **100 % de saturation** (canal vert à zéro sur les trois) et tombaient de 21 % à 9 % de clarté — une teinte pleinement saturée qui plonge vers le noir se lit comme du néon. Désaturés à 52-57 %, départ réchauffé de 337° à 351°, dérive de 22° vers la prune et plancher de clarté remonté à 13 % : `#6b0028 → #2d0011` devient `#7c2734 → #330f22`. Une variante plus chaude encore a été écartée — elle se rapprochait trop du terracotta de l'Accueil. Le bordeaux était **codé en dur dans six fichiers** en plus du thème (planning, aperçu de planning, célébration de course, échelle de chaleur du mode immersif, fond des cartes de record) : tout est aligné. ⚠️ **Le changement est visible sur les cinq onglets**, pas seulement en muscu. ⚠️ Étapes « plan » et « design » traitées en direct (planches comparatives rendues en headless, décision Florian : « tout en une seule vague »). ✅ Aucune migration, aucune sync rule, aucune dépendance native. **Garde-fou** : 5 piliers × 2 thèmes × 21 paires mesurées en non-régression. |
| 3.63 | Refonte du hub Musculation — un écran qui a quelque chose à dire | Le hub répondait à « où en suis-je administrativement », jamais à « est-ce que je progresse ». Neuf surfaces, toutes de la même forme ; trois d'entre elles parlaient de ce qui manque ; le plus gros chiffre était rangé en bas sous un écart sans référence. **Cause mesurée** : sur 36 analyses muscu au catalogue, 20 sont livrées — le hub en montrait **une**, et `selectInsights` (INSIGHTS-01) n'était appelé nulle part côté muscu. | Élevée | ✅ | **MUSCU-UX05 — audit et maquettes validés par Florian le 19/09/2026, code livré le même jour**, en recette → [spec](../specs/functional/us/muscu-ux05-refonte-hub.md) · [RECETTES.md](../../RECETTES.md) §78 (18 critères) · canvas de design « Hub Musculation » (7 planches). 🔴 **Le budget ne bouge pas** : MUSCU-UX01 avait ramené ce hub de 9 blocs à 6 **onze jours plus tôt**, « plus sympa » ne pouvait donc pas vouloir dire « plus de blocs » — neuf surfaces d'administration deviennent **six cartes et deux lignes**. Sortent « Et si… », Volume total, Dernière, Planning ; entrent le **fil du jour** (un insight, pas trois), **Tes charges**, **Ton corps**, **Le mur**. 🔴 **La carte dominante a été refaite après un retour de Florian** : sa première version affichait le **total SBD**, « qui parle à un powerlifter mais pas à un pratiquant de muscu ». Le total SBD est une métrique **de pratique** déguisée en métrique **de progrès** ; la carte mesure désormais les exercices réellement pratiqués, en **trois visages choisis par les données** (gains bruts avant 8 semaines d'historique · médiane des écarts de 1RM estimé · total SBD **mérité**, seulement si les trois mouvements sont désignés ET pratiqués). Un powerlifter retrouve ses trois mouvements sans rien régler. **Deux règles anti-bruit** : médiane et jamais moyenne (un +300 % après blessure ne déplace pas le titre), et séries de **3 à 10 reps** seulement (Epley se dégrade vite au-delà) — le filtre vit dans la brique pour qu'aucun appelant ne l'oublie. ⚠️ **Écart assumé** : le hub **perd sa grille de widgets** et donc sa personnalisation — `strength-widgets.tsx` supprimé, `'strength'` sorti de `WIDGET_SCREENS`, aucune migration (une disposition enregistrée devient simplement illisible). C'est le seul point de la refonte qui **retire** une capacité : à confirmer en recette. ✅ Aucune migration, aucune sync rule, aucune dépendance native. **Vérifié** : typecheck 3 workspaces à 0, lint à 0 **sans warning**, 3746 tests Jest + 159 fichiers Vitest verts. |
| 5.42 | Refonte du hub Course — le bleu sur tout le pilier, et un écran qui sait où en est le coureur | Deux demandes de Florian le 19/09/2026, captures à l'appui : « le héros du cardio est super cool avec ce petit bleu, **le problème c'est que ce n'est pas repris sur le reste des écrans** », et « le dashboard, **je le trouve pas probant** — il faut qu'on sache où on en est et ce qu'il reste à faire ». **Cause mesurée** : sur 25 analyses course au catalogue, 15 sont livrées — le hub en montrait **4** ; ALLURE-01 en avait livré quatre d'un coup le 07/08, aucune n'était remontée ; et `selectInsights` n'était appelé nulle part côté course. Mot pour mot le diagnostic de 3.63, onze heures plus tôt. | Élevée | ✅ | **CARDIO-UX02 — audit et code livrés le 19/09/2026**, en recette → [spec](../specs/functional/us/cardio-ux02-refonte-hub-course.md) · [plan](../plans/cardio-ux02-refonte-hub-course.md) · [RECETTES.md](../../RECETTES.md) §79 (25 critères). 🔴 **Le budget ne bouge pas** : CARDIO-UX01 avait déjà resserré ce hub le 10/09 — **dix surfaces deviennent sept cartes et trois lignes**, et chaque carte **se tait** quand elle n'a rien à dire. Sortent « Personnaliser », la grille de 4 widgets et la bande `RunWeekBand` ; entrent le **fil du jour** (un insight, jamais trois), **Ton allure** (carte dominante, RUN-05), **Ma semaine** (la semaine ET ce qu'il reste), **Ton moteur** (RUN-08, jamais remontée depuis ALLURE-01), **Tes records** (RUN-03, en bande horizontale) et **Ton total** (une ligne). 🔴 **Trois défauts mesurés, pas ressentis.** (1) Le hub et sa propre carte se **contredisaient** sur la même semaine — « 2 / 0 faites » en haut, « 2 / 3 faites » deux blocs plus bas : le repli sur la fréquence visée vivait **dans le composant**, il ne pouvait donc valoir que pour lui. Corrigé à la source (`RunWeekSummary.goalCount`), test de garde vu **rouge** avant. (2) La course était le **seul pilier dont la surface teintée est moins colorée que la surface neutre qu'elle remplace** — chroma **16** contre 18, là où l'accueil est à 36 et la muscu à 29 : `tintPreservingLuminance` désature les teintes froides (le bleu ne pèse que 0,0722 dans la luminance), ce que quatre piliers sur cinq ne pouvaient pas révéler. D'où un **gain de teinte par pilier** (`running: 1.5`) **et** un `PillarPanel` — il manquait une **grande surface** bleue dans le corps de la page, pas de la saturation. (3) **L'identité du pilier dépendait de l'onglet d'où l'on venait** : `useMenuFocus` n'était appelé que par les cinq écrans d'onglet, et l'Accueil pousse vers `/run`, `/run/active` et `/running-history` — entrer par là rendait tout le pilier en **terracotta**. Les 9 écrans empilés déclarent désormais leur pilier, avec un **test de garde de source**. Au passage : le bandeau de record de course était en **bordeaux** (`#7c2734`, la teinte exacte de la scène Musculation, écrite en dur). ⚠️ **Écart assumé** : le hub **perd sa grille de widgets** et donc sa personnalisation, aucune migration — seul retrait de capacité du lot, à confirmer en recette. ⚠️ **Étape design non franchie** : `/design` est réservé à une invocation explicite de Florian et n'est pas lançable depuis une session d'agent ; **rien n'a été produit à la place**. ⚠️ **Les piliers Muscu et Nutrition ont le même défaut d'identité par écran** : constaté, non corrigé (une trentaine d'écrans sans recette), porté au [backlog](../../BACKLOG.md) sous **IDENT-01**. ✅ Aucune migration, aucune sync rule, aucune dépendance native. **Vérifié** : typecheck 3 workspaces à 0, lint à 0 sans warning, **3 741 tests Jest (224 suites) + 3 286 tests Vitest (160 fichiers)** verts, code de sortie lu sans pipe. |
...
| 4.41 | Refonte UX du pilier Nutrition — objectif juste, geste de saisie, journal, suivi, planning | Audit du pilier deux mois apres sa refonte visuelle (4.39). Celle-ci n'avait touche qu'un ecran ; **26 constats verifies dans le code** montrent que le probleme est ailleurs : le pilier a ete construit **fonction par fonction, jamais autour du geste**. Le geste fait cinq fois par jour coutait autant que celui fait trois fois dans une vie. | 🟢 | ✅ | **NUTRI-UX01 — audit et maquettes valides par Florian le 10/09/2026, code livre le meme jour**, en recette → [RECETTES.md](../../RECETTES.md) §58 · [spec](../specs/functional/us/nutri-ux01-refonte-pilier-nutrition.md) · [plan](../plans/nutri-ux01-refonte-pilier-nutrition.md) · [audit](../product/audit-nutrition-2026-09.md) (13 pages) · [maquettes](../../design/nutrition-refonte-2026-09/) (7 planches). **Traite en un seul lot** sur decision de Florian, la ou l'audit en proposait sept. 🔴 **Le defaut le plus grave n'etait pas ergonomique** : l'onboarding ne demandait **jamais** le niveau d'activite, et six sites de code retombaient sur `?? 'moderate'` (×1,55). Pour un sedentaire (×1,2), l'objectif calorique etait **surestime de ~614 kcal/jour** — de quoi annuler tout un deficit de seche, sans qu'aucun ecran ne le signale. `activity_level` devient **nullable** : `null` veut dire « la question n'a jamais ete posee », et le repli devient *visible* au lieu de se faire passer pour un choix. **Le geste quotidien** : le selecteur passe de **9 entrees de meme poids** (5 onglets + 4 boutons de pied) a une **feuille a 3 modes** — c'est l'arbitrage ecrit dans la maquette du 30/07/2026 et jamais pose dans le code. La liste ouvre sur **les habitudes** et non sur la base alphabetique ; la recherche est **unifiee** (aliments + recettes + repas types, spec §5.2 enfin respectee) et **classee par pertinence** avec tolerance aux fautes — `bestMatchIndex` existait et n'etait branche nulle part ; le **budget du jour ne quitte plus l'ecran** ; les portions deviennent **multipliables** et la **derniere quantite utilisee** est relue (`food_entries.quantity_g`, jamais lue jusqu'ici). **Trois exigences de spec enfin tenues** : le **calendrier mensuel** avec jours renseignes (§4.7), les **sorties de secours** de la saisie texte (§4.5 — une ligne non reconnue etait un cul-de-sac), et la mention **cru / cuit** (§8 — riz cru contre riz cuit, facteur 3 sur les calories). **Hydratation ouverte en V1** (NUTR-12, contre la spec §8 qui la reportait) : un tap, une table, aucune donnee a saisir. **Micros actifs par defaut** — `tracked: []` rendait invisible le seul vrai differenciateur du pilier (33 micros CIQUAL + VNR). **Suivi** passe de **8 sections + 4 cartes** a **4 sous-onglets** (ADR-007 §2, qui nommait deja cet ecran comme le point de saturation) ; la regularite devient une heatmap, l'adherence un graphe a zone-cible. **Planning** : il n'acceptait que recettes et repas types, donc planifier un premier repas imposait d'aller creer une recette (~20 taps) — il accepte desormais un **aliment simple** et un **ajout rapide**, et s'ouvre en **grille de semaine**. 🔴 **3 migrations + 1 publication** : `water_entries` (table neuve), `meal_plan_entries` (`food`/`quick` + `food_id`/`quantity_g`), `foods.preparation_state`, `nutrition_profiles.activity_level` nullable. **1 sync rule neuve a deployer A LA MAIN** (`water_entries`) — etape deja oubliee trois fois. ✅ **La bibliotheque est remplie depuis le 13/09/2026** : 80 → **3 244 aliments**, import CIQUAL 2025 (ANSES, Licence Ouverte) par `generate.py --bulk`, **aucune valeur inventee**. A la livraison du 10/09 ce lot etait **materiellement impossible** — le CSV CIQUAL n'est pas versionne et etait absent de la machine ; il a ete telecharge depuis l'entrepot recherche.data.gouv.fr. Le passage a l'echelle reelle a revele **trois defauts du generateur invisibles sur 80 lignes** (libelles de groupes faux : ni viande, ni poisson, ni laitage, ni boisson n'entrait dans l'import ; id de traduction positionnels, invalides au-dela de 999 aliments ; `--limit` qui coupait dans l'ordre du fichier) **et un plafond de recherche devenu faux** : `SQL_SCAN_LIMIT` passe de 400 a 800, et les correspondances par **debut de nom** passent devant — sur 3 244 aliments, « pomme » disparaissait a « po » pour revenir a « pom », une liste qui retrecit quand on precise sa recherche. **Reste ouvert** : la traduction EN des 3 164 noms importes (CIQUAL est monolingue) et leurs portions usuelles. **Verifie** : typecheck 3 workspaces a 0, lint a 0 (0 warning), **2 748 tests verts**. |
| 4.42 | Dépense énergétique d'une séance — le moteur | Une seule formule pour les trois sources (muscu, course, activité) : **(MET − 1) × métabolisme de repos par heure × heures actives**. L'âge, la taille, le poids et le sexe entrent par le métabolisme de repos ; le **niveau n'entre pas**, et c'est une décision (il agit déjà par les charges, le rythme et l'allure). Tout sort en **fourchette** avec un niveau de confiance. | 🟢 | ✅ | **DEPENSE-01 — livrée le 15/09/2026**, en recette → [spec](../specs/functional/us/depense01-moteur-depense.md) · [analyse](../product/analyse-depense-activites-2026-09.md) · [maquettes](../../design/depense-activites-2026-09/) · [RECETTES.md](../../RECETTES.md) §65. **Répare trois trous de RN-01** : le dénivelé était **ignoré** (un trail 15 km / D+ 800 était estimé comme du plat), une course **sans GPS valait 0 kcal** (mode tapis, roadmap 5.21), et le poids retenu était le **dernier connu** — donc les sorties passées étaient réévaluées à chaque pesée, et l'adhérence calorique passée changeait après coup. 🔴 **Non-régression figée par test** : sans dénivelé, le résultat reste celui d'`estimateRunCalories`. ⚠️ **MET du Compendium 2011**, à revérifier dans la table 2024 ; largeurs de fourchette non calibrées (±15/25/30 %). ✅ 100 % de couverture sur les deux fichiers neufs. |
| 4.43 | Autres activités — vélo, natation, rando… | L'app ne connaissait que **deux** types d'activité, partout : série, jour d'entraînement, charge, temps d'entraînement, Health Connect. Trois heures de vélo comptaient pour un **jour de repos** et **cassaient la série**. Saisie à la main (type, durée, intensité au test de la parole), catalogue de 22 sports, et branchement sur les cinq surfaces qui l'ignoraient. | 🟢 | ✅ | **AUTRE-01 — livrée le 15/09/2026**, en recette → [spec](../specs/functional/us/autre01-autres-activites.md) · [RECETTES.md](../../RECETTES.md) §65. **Table neuve `activities`** (migration poussée le 15/09) — 🔴 **sync rule à coller à la main**, table synchronisée. 🔴 **Le ressenti est prérempli depuis l'intensité** : sans lui, la charge sRPE de l'activité vaudrait zéro et l'app dirait « repos » après 90 km de vélo. 🔴 **La dépense n'est pas stockée** : recalculée avec le **poids à la date** (décision D3). ⚠️ **Ni course ni muscu au catalogue** (elles ont leur pilier), **ni ménage ni jardinage** (déjà dans le socle hors sport). ⚠️ Écart assumé : **pas d'entrée dans la barre d'actions rapides** de l'accueil, limitée à 4 pastilles par ACCUEIL-03. Health Connect reçoit la séance **sans calories** — en ajouter demanderait un 7ᵉ type dans la déclaration Play, qui ne se dépose qu'une fois. |
| 4.44 | La cible calorique qui suit les dépenses réelles | Le **double comptage** : le facteur d'activité du TDEE est défini par la fréquence d'entraînement (RN-03 : « modéré » dès 3 séances/semaine), et le bonus Auto (RN-02) rajoutait la dépense par-dessus — jusqu'à **95 % du déficit d'une sèche effacé**, en silence. Troisième mode de cible (**socle hors sport + dépenses réelles**), carte « Ta journée en énergie », et réglage pour masquer les chiffres. | 🟢 | ✅ | **DEPENSE-00 / 02 / 03 — livrées le 15/09/2026**, en recette → [spec](../specs/functional/us/depense00-socle-hors-sport.md) · [RECETTES.md](../../RECETTES.md) §65. 🔴 **Personne ne bascule tout seul** : aucune migration de données, les modes `fixed` et `auto` sont **inchangés à la ligne près** (tests de non-régression) — changer une cible calorique en silence serait le défaut qu'on répare. 🔴 **Le mode neuf change AUSSI le socle** (`sportFreeTdee`), il n'ajoute pas qu'un bonus. 🔴 **La cible ne retient que le bas de la fourchette** (décision D2) : les MET surestiment plus souvent qu'ils ne sous-estiment. ⚠️ **Masquer les chiffres n'éteint pas le calcul** — on retire l'affichage, pas le moteur. ⚠️ Écarts assumés : pas d'aperçu avant/après chiffré au basculement, pas d'anticipation d'une séance planifiée dans ce mode, paliers hors sport repris provisoirement des multiplicateurs existants. |
| 4.40 | Manges-tu comme tu t'entraînes ? — lot d'analyses croisées muscu × nutrition | L'app sait ce que tu manges et comment tu t'entraînes, mais ne dit **jamais si l'un va avec l'autre**. Quatre analyses : **bilan énergétique** jours de séance vs repos, **adhérence** aux macros comparée entre les deux, **disponibilité énergétique** les jours de gros volume, et **protéines fractionnées** dans la journée. On met deux chiffres côte à côte ; on n'affirme aucune causalité. | 🟢 | ✅ | **APPORT-01 — validée et livrée le 08/08/2026**, en recette → [RECETTES.md](../../RECETTES.md) §53 · [spec](../specs/functional/us/apport01-manger-comme-on-sentraine.md) · [plan](../plans/apport01-manger-comme-on-sentraine.md) · [maquette](../../design/apport01-manger-comme-on-sentraine/apport01-manger-comme-on-sentraine.html). **3ᵉ lot du [catalogue](../product/analyses-donnees.md)** (MN-20, MN-16, MN-15, MN-10) après EXEC-01 (muscu) et ALLURE-01 (running) — cette fois le **croisé**, c'est-à-dire le différenciateur revendiqué par la [vision](../product/vision.md). 🟢 **Ce lot ne réinvente presque rien** : `isTrainingDay`, `computeGoalAdherence`, `computeCaloricBalance`, `resolveMealSplit` et `computeVolume` sont livrés. 🔴 **Trois calibrages sont RÉUTILISÉS et non recopiés** — la définition d'un jour d'entraînement (règle non triviale : le passé n'est jamais anticipé), la **marge d'adhérence qui est un réglage utilisateur** (`adherenceMarginPct`, défaut 10 — en coder une en dur donnerait deux taux contradictoires dans la même app), et le groupement par repas de NUTR-16. **Un seul nombre inventé** dans tout le lot : le facteur de « gros volume », et encore relatif à la **médiane personnelle**. ⚠️ **Asymétrie délibérée** : une course compte comme jour d'entraînement pour le bilan et l'adhérence, mais produit zéro volume muscu — donc jamais un « gros volume ». ⚠️ **Aucune causalité affirmée** : les 6 items du catalogue qui corrèlent apport et progression de force relèvent du moteur de corrélations, hors lot. ⚠️ Sans pesée, les g/kg n'existent pas et **aucune valeur neutre** ne les remplace : la carte reste et affiche son remède. ✅ Aucune migration, aucune sync rule, aucune écriture → recettable sur l'APK existant. |
...
| **Total périmètre de lancement** | **259** | |
| ⏳ Reporté (section « Ultérieur — iOS » : 9.1, 1.3) | 2 | *hors décompte* |

> **CARDIO-UX02, 19/09/2026** ajoute **5.42** (refonte du hub Course) : 246 → 247 livré, total 258 → 259. Même passe que 3.63 sur le pilier voisin, onze heures plus tard. Le budget de blocs est tenu ; le hub course perd sa grille de widgets — seul retrait de capacité, à confirmer en recette. Trois défauts **mesurés** corrigés au passage : un dénominateur de semaine contradictoire, la teinte de pilier la plus délavée des cinq, et une identité de pilier qui dépendait du chemin d'entrée.

> **MUSCU-UX05, 19/09/2026** ajoute **3.63** (refonte du hub Musculation) : 245 → 246 livré, total 257 → 258. Née d'une demande de Florian (« ce dash doit donner plus envie de rester »). Le budget de blocs est tenu ; le hub perd en revanche sa grille de widgets — seul retrait de capacité du lot, à confirmer en recette.

> **MUSCU-UX04, 19/09/2026** ajoute **3.62** (identité d'un pilier portée par toute la page) : 244 → 245 livré, total 256 → 257. Née d'un constat de recette le jour même (« les cartes en dessous, c'est pas du tout ISO ») ; corrige au passage un accent de réglage à 1,15:1. **MUSCU-FIX01** ne crée pas de ligne : trois correctifs de flux notés sur **3.59**, dont une régression de cette US (point d'entrée des templates jamais construit).
...
> **241 → 242 le 18/09/2026 (ter)** : la ligne **7.32**, LETTRE-01. Dernière des trois lignes du
> **lot 1 du carnet d'innovation** (avec 5.41 et 4.45), et la plus petite : trois colonnes, aucun
> moteur, aucune notification. Elle est comptée parce qu'elle livre une surface visible — la règle
> du dépôt étant de ne jamais laisser du livré invisible.
...
> 13/09/2026 a relevé qu'ils étaient les seuls de l'app à n'avoir **aucune identité de pilier** :
> le même empilement de cartes blanches quatre fois, là où chaque pilier a sa couleur, sa matière et
> son moment. Elle emporte aussi la première brique d'IA du dépôt, **désactivée par défaut**.

> **247 → 248 le 15/09/2026 (ter)** : la ligne **7.31**, IA-LAB-01. Elle ne rallonge pas le
> périmètre de lancement — la surface est **hors build**, derrière un opt-in éteint. Elle est
> comptée parce que la règle du dépôt est de **ne jamais laisser du livré invisible**, et parce
> qu'elle referme un angle mort du dossier IA : l'[analyse du 15/07](../product/ia-integration-analyse.md)
> avait tout chiffré sauf **ce que le modèle rend vraiment**. Le §7 « Tester en gratuit » y a été
> ajouté le même jour. Deux choix la caractérisent : le **fournisseur devient un réglage**
> (`AI_PROVIDER`, adaptateurs Gemini et Anthropic), et le jeu de test **plante six signaux connus**
> — sans vérité de référence, une réponse d'IA se commente mais ne se note pas.
>
> **246 → 247 le 15/09/2026 (bis)** : la ligne **7.30**, LABO-01. Elle ne rallonge pas le périmètre au
> sens d'un nouveau besoin — elle donne **une maison** aux 28 analyses inter-piliers déjà livrées,
> que le [catalogue](../product/analyses-donnees.md) compte parmi 87, et qui vivaient éparpillées en
> bandeaux et en cartes. Même geste qu'INSIGHTS-01 (7.20), à l'autre bout : Insights **raconte** ce
> qui mérite d'être vu, le Labo **fait faire**. Elle emporte une décision d'architecture neuve
> ([ADR-008](../adr/ADR-008-scene-3d-composant-dom.md), premier composant DOM Expo du dépôt) et la
> note de nuit du check-in, sans toucher la déclaration Play Store.
>
> **241 → 242 le 13/09/2026 (bis)** : création de la ligne **3.61** (MUSCU-UX03, mode immersif
> de la séance) dans « Hors périmètre de cadrage », **livrée dans la foulée**. Née d'une demande
> de Florian (12/09) d'une séance plus immersive, puis d'une décision du 13/09 : **c'est un mode
> en plus, le classique reste**. Les dix lots du plan sont écrits (mode et réglages, briques pures
> dans `packages/shared`, écran immersif, effort, cadran, repos vivant, retours, cérémonie de fin,
> coach vocal, brief et fil) ; le mode classique ne gagne que la pastille de record au repos
> (décision D3). 56 critères de recette en [RECETTES.md §63](../../RECETTES.md).
> **Livré ≠ validé** : la recette device reste à faire. Hors cadrage passe de 37 à 38 lignes.
>
...
> 20 défauts référencés au fichier et à la ligne, dont trois écarts à des specs déjà écrites
> (`navigation-ux.md` §3.1 pour le poids, l'heure de séance d'HORAIRE-01, la date de la maquette
> validée). Elles sont comptées parce que la règle du dépôt est de **ne jamais laisser du livré
> invisible**.

> **236 → 237 le 10/09/2026** : la ligne **3.59**, refonte UX du pilier Musculation (MUSCU-UX01).
> Même logique que les six précédentes, et **même semaine** : après l'écran d'atterrissage, le
> pilier le plus utilisé. 17 constats vérifiés dans le code, dont trois promesses de spec jamais
> tenues (retour haptique de `navigation-ux.md` §4.2, durée estimée de `musculation.md` §4.2,
> suppression d'une séance passée §6.1) — ce ne sont donc pas des ajouts de périmètre, mais des
> écarts comblés. Elle est comptée pour la même raison que les autres.

> **237 → 238 le 10/09/2026** : la ligne **4.41**, refonte UX du pilier Nutrition (NUTRI-UX01).
> Troisième pilier repris dans la même semaine, après l'accueil et la musculation. 26 constats
> vérifiés dans le code, dont **quatre exigences de spec jamais tenues** (le calendrier mensuel
> d'`alimentation.md` §4.7, les sorties de secours de la saisie texte §4.5, la mention cru/cuit
> §8, les portions sur un aliment perso §4.3) et **un dépassement de l'ADR-007** que l'ADR
> lui-même avait signalé comme imminent sur cet écran. Ce ne sont donc pas des ajouts de
> périmètre mais des écarts comblés — sauf l'**hydratation** (NUTR-12), seule vraie ouverture,
> décidée par Florian contre le report en V2 de la spec §8.
> 🔴 **Ce que la ligne ne couvre pas** : la bibliothèque compte toujours **80 aliments**. Le
> remplissage n'est pas un choix de périmètre mais une impossibilité matérielle (CSV CIQUAL
> absent de la machine, cf. spec §D1) ; l'outillage qui le rend trivial est livré, la donnée non.
> C'est **le premier reste-à-faire du pilier**, et il ne demande pas de développement.

> **238 → 239 le 10/09/2026** : la ligne **5.40**, refonte UX du pilier Course (CARDIO-UX01).
> Troisième audit de pilier de la semaine, et la même règle que les sept lignes précédentes : on ne
> laisse pas du livré invisible. Elle entre en **🟡 Partiel**, ce qui est nouveau pour une ligne de
> refonte — les 5 constats bloquants sont corrigés, six chantiers d'ergonomie restent ouverts et
> sont nommés dans la section de recette. Une partie des 40 constats sont des **écarts à des
> promesses déjà écrites**, pas des ajouts de périmètre : la ligne 5.13 annonce « GPS + chronomètre
> **d'un tap** » et un « compte à rebours optionnel » jamais implémentés, et la ligne 5.21 annonce
> un « suivi à la durée seule » qui n'enregistrait rien.
...
| Hors cadrage (52) | 51 | 1 | 0 | 0 | 0 | **Recompté ligne à ligne le 18/09/2026** (50 lignes, 49 ✅, 1 🟡) : la valeur « 42 » datait du 16/09 et n'avait suivi ni LABO-01/IA-LAB-01 ni le lot 1 du carnet d'innovation — FANT-01 (5.41), RESERV-01 (4.45), LETTRE-01 (7.32). MOTION-01 (3.60) reste le seul partiel. |
...
**19/09/2026 (bis) — CONS-01, le Conseil des trois (fin du lot IA)**
Nouvelle ligne **7.34**. Compteur livré 243 → 244, total 255 → 256. Le « moteur de compromis
inexistant » de GUID-01 était déjà là, sous le nom de `lab-composer`. En recette (§75).

**19/09/2026 — NARR-01, l'IA raconte le dossier (début du lot IA)**
Nouvelle ligne **7.33**. Compteur livré 242 → 243, total 254 → 255. L'Enquête étant déjà livrée par
LABO-01, l'US se réduit à la narration + son garde-fou. En recette (§74).

**18/09/2026 (ter) — LETTRE-01, la lettre à son futur soi (fin du lot 1)**
Nouvelle ligne **7.32** en « Hors périmètre de cadrage ». Compteur livré 241 → 242, total 253 → 254.
Ligne « Hors cadrage » du détail par version **recomptée** : 50 lignes, 49 ✅. En recette (§73).
...
13/09, pas du cadrage. Compteur livré 239 → 240. En recette (RECETTES §71).

**16/09/2026 — Réunion du lot « Mon corps » avec `dev`**
Fusion de LABO-01 (7.30) et du lot DEPENSE (4.42-4.44) dans le lot CORPS-01→04. Compteurs recalculés
depuis les lignes : **239 livrés / 4 partiels / 2 à faire sur 251**. Recettes CORPS renumérotées **§68 / §69**
(collision : `dev` occupait déjà §65 et §66).
**16/09/2026 — CORPS-04 (6.7) : programme compatible expliqué**
Nouvelle ligne ✅ : classement local par priorités et contraintes, préparation d'une copie inactive ; recette §69 groupée avec CORPS-03.
Compteurs recalculés depuis les lignes : **234 livrés / 4 partiels / 2 à faire sur 246**. Deux reportés et quatre abandonnés.

**15/09/2026 — LABO-01 (7.30) : les 28 analyses inter-piliers ont enfin une maison**
Périmètre **247** (+1), ✅ passe de 234 à 235. Aucune analyse nouvelle n'est calculée : le Labo
**fait faire** là où Insights **raconte**, et rien ne s'écrit sans la feuille de confirmation.
Emporte [ADR-008](../adr/ADR-008-scene-3d-composant-dom.md) (1ᵉʳ composant DOM Expo) et la note de
nuit du check-in — **déclaration Play inchangée**. Reste 1 sync rule à déployer et la recette (§66).

> **241 → 244 le 15/09/2026** : les trois lignes **4.42 · 4.43 · 4.44** — la dépense d'une séance,
> les autres activités, et la cible qui les suit. Elles ne rallongent pas le périmètre au sens d'un
> nouveau besoin : elles **réparent un défaut mesuré** (le sport compté deux fois dans la cible
> calorique, jusqu'à 95 % du déficit d'une sèche effacé) et comblent un angle mort structurel
> (l'app ne connaissait que deux types d'activité, partout).
> **Livré ≠ validé** : 33 critères de recette en [RECETTES.md §65](../../RECETTES.md), et une
> **sync rule à coller à la main** avant de commencer. Hors cadrage passe de 38 à 41 lignes.
...
Compteurs avant réunion avec `dev` : **230 livrés / 4 partiels / 2 à faire sur 242**. Adaptation automatique ultérieure.
...
Chemins CMake raccourcis via plugin Expo ; APK vérifié. Compteurs inchangés : 229 / 4 / 2 sur 241.

**13/09/2026 — CORPS-02 (6.5) : silhouette personnelle et intention visuelle**
Nouvelle ligne ✅ : départ paramétrique, objectif indépendant, comparaison et sauvegarde offline ; recette Android §63.
Compteurs : **229 livrés / 4 partiels / 2 à faire sur 241**. Migration additive appliquée, aucune nouvelle sync rule.

**13/09/2026 (ter) — NUTRI-UX01 (4.41) : la bibliothèque passe de 80 à 3 244 aliments**
Aucun changement de périmètre ni de compteur — la ligne était déjà ✅. Le seul point resté
ouvert de la refonte est comblé : import CIQUAL 2025, **aucune valeur inventée**. Le fichier
réel a révélé 3 défauts du générateur et 1 plafond de recherche devenu faux — voir CHANGELOG.

**13/09/2026 — DASH-01 (7.29) : les quatre dashboards passent à la scène, en une vague**
Périmètre **241** (+1), ✅ passe de 228 à 229. Les quatre écrans d'atterrissage étaient les seuls de
l'app sans identité de pilier ; ils ont désormais chacun leur couleur, leur matière et leur moment.
Emporte la 1ʳᵉ brique d'IA du dépôt — **désactivée par défaut**, et aucun chiffre affiché n'en sort.

**12/09/2026 — MOTION-01 (3.60) : une ligne neuve, 🟡 partiel assumé**
Périmètre **240** (+1) et 🟡 passe de 3 à 4. La ligne naît directement en 🟡 : **19 effets livrés
sur les 45 analysés**, socle complet, 4 effets écartés pour une raison de fond (spec §4.6).
...
Carte ombrée partagée, sélection / caméra accessibles, exercices offline et six accès ; recette Android §62 en attente.
...
et non un oubli : Expo SDK 54 force l'edge-to-edge, `adjustResize` ne redimensionne plus rien, et
tout écran qui comptait dessus voit son clavier recouvrir le contenu (cf. CLAV-01 au backlog).

**11/09/2026 — MUSCU-UX01 (3.59) : 1ʳᵉ passe de recette, 7 constats corrigés**
Compteurs **inchangés** — la ligne reste ✅ et l'US reste en recette. Deux des sept étaient des
requêtes SQL invalides qu'aucun test ne pouvait voir : sous PowerSync l'erreur est avalée et
l'écran affiche un état « pas de donnée » légitime. Un sweep couvre désormais les 162 requêtes.

**10/09/2026 — ACCUEIL-01 → 06 : recette validée, 6 US clôturées (7.23 → 7.28)**
Compteurs inchangés : **226 livré / 2 partiel / 2 à faire sur 236** (~96 %). Une passe de recette
device a trouvé 5 défauts, dont un qui rendait 2 des 8 états de la carte « maintenant »
inatteignables — aucun n'était détectable par les tests, qui injectent tous leurs entrées.

**09/09/2026 — ACCUEIL-01 → 06 : refonte de l'écran d'accueil (6 lignes créées, 7.23 → 7.28)**
Compteurs : **226 livré / 2 partiel / 2 à faire sur 236** (~96 %). Périmètre **jamais cadré** : la
refonte corrige 20 défauts de l'écran d'atterrissage, dont 3 écarts à des specs déjà écrites (poids
de `navigation-ux.md` §3.1, heure d'HORAIRE-01, date de la maquette). `MAX_HOME_WIDGETS` inchangé.
...
auparavant — la roadmap n'a pas régressé, elle s'étend : la 5.9 demandait « 6×400 m à 95 % VMA »
et c'est bien ce que RUN-F2c avait livré. 5.39 reste 🟡 : calcul et données livrés, surface partielle.
...
aux aliments récents — comblé par un pré-filtrage **SQL** borné, sans charger CIQUAL en mémoire.
Rouvert non par la recette mais par un constat : **au lancement, aucun compte n'a d'aliment récent**.
...
**08/08/2026 — APPORT-01 (4.40) : livrée, ⬜ → ✅**
4 moteurs purs à 100 %, 2 requêtes, 1 section repliée sur l'écran Nutrition. 214 livré / 2 à faire sur
226. D1 **affinée en codant** : `trainedDays` plutôt qu'`isTrainingDay`, pour que le groupement colle
exactement à la cible. Recette : 19 critères, §53.
