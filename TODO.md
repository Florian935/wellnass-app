...
- [~] **Modifier / supprimer un aliment ajouté à un repas — geste peu découvrable + édition limitée à la
  quantité** — _remontée Florian, 16/07/2026._ **Spec + plan validés Florian (16/07/2026)** →
  implémentation à venir (subagent-driven, branche `fix/journal-entree-swipe-edition`). **Design retenu**
  : swipe gauche sur l'entrée → Modifier + Supprimer (tap conservé, appui long retiré) ; édition élargie
  aux quick add (kcal/P/G/L/nom), entrées avec quantité inchangées (grammes). 100 % client, aucune
  migration. Spec : [us/fix-journal-entree-swipe-edition.md](docs/specs/functional/us/fix-journal-entree-swipe-edition.md) ·
