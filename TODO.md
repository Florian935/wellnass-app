...
*Dernière mise à jour : 11/07/2026 (Running R2 carte : code livré et revu → reste recette device)*
...
- [~] **Running R2 — Carte** (5.17/5.27) — **code livré & revu** (`feature/running-r2-carte`, MapLibre + MapTiler, [ADR-006](docs/adr/ADR-006-cartographie.md)). `simplifyTrack` (Douglas-Peucker, shared, testé) + `RouteMap` réutilisable (live `follow` / résumé fit-bounds) + `RunDetail.gpsTrack` + branchement `active`/`summary` + i18n FR/EN. Cadrage complet (spec→plan→maquette validés), revues par phase + finale (*Approved*). typecheck/lint/tests verts. **Reste : recette device** (nouveau dev/preview build MapLibre + **clé MapTiler** à fournir/pousser en EAS env). Différé : tuiles offline, sélecteur de style, export GPX (R4).
