...
- [ ] **US 8.8b — Bannissement des utilisateurs** — **à cadrer (avec Damien)** : RPC `SECURITY DEFINER`
  sur `auth.users.banned_until` (ban réel, coupe la connexion + PowerSync) + table `user_bans` (motif) +
  actions bannir/débannir (rôle moderator) + audit. Sensible (écriture schéma `auth`). Mécanisme tranché
  (Florian, 16/07/2026) = RPC SQL, à confirmer techniquement avec Damien.
